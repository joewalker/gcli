...
exports.exec = function(command) {
  return requisition.update(command).then(function() {
    if (requisition.getStatus() !== Status.VALID) {
      throw new Error(requisition.getStatusMessage());
    }
    else {
      var output = requisition.exec();

      function convert() {
        return converters.convert(output.data, output.type, 'string',
                                  requisition.conversionContext);
      }

      return output.promise.then(function() { return convert(); },
