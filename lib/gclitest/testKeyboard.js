...
function check(initial, action, after, choice, cursor, expectedCursor) {
  ...
  if (inputter) {
    ...
    // TODO: This will need 'then', which probably means moving this code
  ...
  switch (action) {
    case COMPLETES_TO:
      finished = requisition.complete({ start: cursor, end: cursor }, choice);
      break;
  ...
  Q.resolve(finished).then(check, console.error);
}
