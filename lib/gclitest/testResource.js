...
function checkPrediction(res, prediction) {
  ...
  test.is(conversion.value, value, 'value wrong for ' + name);

  var strung = res.stringify(value);
  test.is(strung, name, 'stringify wrong for ' + name);

  test.is(typeof value.loadContents, 'function', 'not a resource for ' + name);
}
