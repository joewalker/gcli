...
function exec(command, expectedArgs) {
  ...
  assert.is(command.indexOf(actualExec.command.name), 0,
            'Command name: ' + command);

  assert.is(command,
            outputObject.typed,
            'outputObject.command for: ' + command);
  assert.ok(outputObject.completed,
            'outputObject.completed false for: ' + command);
  ...
  assert.is(Object.keys(expectedArgs).length,
            Object.keys(actualExec.args).length,
            'Arg count: ' + command);
