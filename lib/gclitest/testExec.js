...
function onCommandExec(ev) {
  actualArgs = ev.args;
  actualContext = ev.context;
  actualCommand = ev.command;
}
...
function exec(command, expectedArgs) {
  ...
  test.is(command.indexOf(actualCommand.name), 0, 'Command name: ' + command);
  ...
  if (actualArgs == null) {
    test.ok(false, 'actualArgs == null for ' + command);
    return;
  }

  test.is(Object.keys(expectedArgs).length, Object.keys(actualArgs).length,
          'Arg count: ' + command);
  Object.keys(expectedArgs).forEach(function(arg) {
    var expectedArg = expectedArgs[arg];
    var actualArg = actualArgs[arg];
  ...
  test.is(environment, actualContext.environment, 'Environment');
}
