...
helpers._actual = {
  ...
  predictions: function() {
    ...
    return Q.resolve(assignment.getPredictions()).then(function(predictions) {
...
helpers._createDebugCheck = function() {
  ...
  return util.promised(Array)(hintsPromise, predictionsPromise).then(function(values) {
    ...
    var output = '';
    var position = (cursor === input.length) ? '' : ', ' + cursor;
    output += 'helpers.setInput(\'' + input + '\'' + position + ');\n';
    ...
    if (requisition._unassigned.length === 0) {
      ...
      output += '  unassigned: [\'' + requisition._unassigned.join('\',\'') + '\'],\n';
    }
...
helpers.check = function(checks) {
  ...
  if ('hints' in checks) {
    var checkHints = test.checkCalled(function(actualHints) {
      assert.is(actualHints, checks.hints, 'hints');
    });
    Q.resolve(helpers._actual.hints()).then(checkHints, console.error);
  }

  if ('predictions' in checks) {
    var checkPredictions = test.checkCalled(function(actualPredictions) {
      helpers._arrayIs(actualPredictions, checks.predictions, 'predictions');
    });
    Q.resolve(helpers._actual.predictions()).then(checkPredictions, console.error);
  }

  if ('predictionsContains' in checks) {
    var checkContains = test.checkCalled(function(actualPredictions) {
      checks.predictionsContains.forEach(function(prediction) {
        var index = actualPredictions.indexOf(prediction);
        assert.ok(index !== -1, 'predictionsContains:' + prediction);
      });
    });
    Q.resolve(helpers._actual.predictions()).then(checkContains, console.error);
  }
  ...
  if ('unassigned' in checks) {
    helpers._arrayIs(helpers._actual.unassigned(), checks.unassigned, 'unassigned');
  }
