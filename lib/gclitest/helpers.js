 ...
 * - check: Check data to be passed to helpers.check()
...
helpers.audit = function(audits) {
  var auditNext = function(index) {
    ...
    if (typeof audit.setup === 'string') {
      promise = helpers.setInput(audit.setup);
    }
    else if (typeof audit.setup === 'function') {
      promise = Q.resolve(audit.setup());
    }
    else {
      promise = Q.resolve(undefined);
    }

    promise.then(function() {
      helpers.check(audit.check);
