...
var actual = {
  input: function() {
    return cachedOptions.display.inputter.element.value;
  },

  hints: function() {
    var actual = cachedOptions.display.completer._getCompleterTemplateData();
  ...
  markup: function() {
    var display = cachedOptions.display;
    var cursor = display.inputter.element.selectionStart;
    var statusMarkup = display.requisition.getInputStatusMarkup(cursor);
  ...
  cursor: function() {
    return cachedOptions.display.inputter.element.selectionStart;
  },

  current: function() {
    return cachedOptions.display.requisition.getAssignmentAt(actual.cursor()).param.name;
  },

  status: function() {
    return cachedOptions.display.requisition.getStatus().toString();
  },

  outputState: function() {
    var outputData = cachedOptions.display.focusManager._shouldShowOutput();
    return outputData.visible + ':' + outputData.reason;
  },

  tooltipState: function() {
    var tooltipData = cachedOptions.display.focusManager._shouldShowTooltip();
    return tooltipData.visible + ':' + tooltipData.reason;
  }
};

var directToString = [ 'boolean', 'undefined', 'number' ];

exports.createDebugCheck = function() {
  ...
  output += '  input:  \'' + input + '\',\n';
  output += '  hints:  ' + padding + '\'' + actual.hints() + '\',\n';
  output += '  markup: \'' + actual.markup() + '\',\n';
  output += '  cursor: ' + actual.cursor() + ',\n';
  output += '  current: \'' + actual.current() + '\',\n';
  output += '  status: \'' + actual.status() + '\',\n';
  output += '  outputState: \'' + actual.outputState() + '\',\n';

  if (command) {
    output += '  tooltipState: \'' + actual.tooltipState() + '\',\n';
    output += '  args: {\n';
    ...
    requisition.getAssignments().forEach(function(assignment) {
      ...
      else if (directToString.indexOf(typeof assignment.value) !== -1) {
        output += 'value: ' + assignment.value + ', ';
      }
...
exports.setInput = function(typed, cursor) {
  ...
  cachedOptions.display.focusManager.onInputChange();
};
...
exports.focusInput = function() {
  cachedOptions.display.inputter.focus();
};
...
exports.pressTab = function() {
  exports.pressKey(util.KeyEvent.DOM_VK_TAB);
};

/**
 * Simulate pressing RETURN in the input field
 */
exports.pressReturn = function() {
  exports.pressKey(util.KeyEvent.DOM_VK_RETURN);
};
...
exports.check = function(checks) {
  if ('input' in checks) {
    test.is(actual.input(), checks.input, 'input');
  }

  if ('cursor' in checks) {
    test.is(actual.cursor(), checks.cursor, 'cursor');
  }

  if ('current' in checks) {
    test.is(actual.current(), checks.current, 'current');
  }

  if ('status' in checks) {
    test.is(actual.status(), checks.status, 'status');
  }

  if ('markup' in checks) {
    test.is(actual.markup(), checks.markup, 'markup');
  }

  if ('hints' in checks) {
    test.is(actual.hints(), checks.hints, 'hints');
  }

  if ('tooltipState' in checks) {
    test.is(actual.tooltipState(), checks.tooltipState, 'tooltipState');
  }

  if ('outputState' in checks) {
    test.is(actual.outputState(), checks.outputState, 'outputState');
  }
