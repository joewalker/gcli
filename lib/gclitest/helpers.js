...
helpers.pressKey = function(keyCode) {
  ...
  return helpers._display.inputter.handleKeyUp(fakeEvent);
};
 ...
 */
helpers.reason = {
  jsdom: function jsdom(options) {
    return options.isJsdom;
  },
  phantomjs: function phantomjs(options) {
    return options.isPhantomjs;
  },
  firefox: function firefox(options) {
    return options.isFirefox;
  },
  createComandCheck: function(commandName) {
    return function commandMissing(options) {
...
helpers.audit = function(audits) {
  ...
  return util.promiseEach(audits, function(audit) {
    ...
    if (audit.skipIf && audit.skipIf(helpers._options)) {
      var reason = audit.skipIf.name ? 'due to ' + audit.skipIf.name : '';
      assert.skip('Skipped ' + name + ' ' + reason);
      return Q.resolve(undefined);
    }

    if (audit.skipRemainingIf && audit.skipRemainingIf(helpers._options)) {
