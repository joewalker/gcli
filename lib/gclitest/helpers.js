...
helpers.setInput = function(typed, cursor) {
  ...
  if (typed.indexOf('<') === -1) {
    ...
    promise = inputter.setInput('').then(assert.checkCalled(function() {
      return promiseEach(chunks, assert.checkCalled(function(chunk) {
        if (chunk.charAt(0) === '<') {
  ...
  return promise.then(assert.checkCalled(function() {
    if (cursor != null) {
 ...
 * @return A promise which resolves to undefined when the checks are complete
 */
helpers._checkExec = function(name, expected) {
...
helpers.audit = function(audits) {
  return promiseEach(audits, assert.checkCalled(function(audit) {
    var name = audit.name;

    var setupDone = undefined;
    if (typeof audit.setup === 'string') {
      name = name || audit.setup;
      setupDone = helpers.setInput(audit.setup);
    }
    else if (typeof audit.setup === 'function') {
      setupDone = audit.setup();
    }

    // console.log('-  \'' + name + '\' in ' + assert.currentTest);

    return Q.resolve(setupDone).then(function() {
      return helpers._check(name, audit.check).then(assert.checkCalled(function() {

        var execDone = (audit.exec != null) ?
            helpers._checkExec(name, audit.exec) :
            Q.resolve();

        return execDone.then(assert.checkCalled(function() {
 ...
 */
function promiseEach(array, action, scope) {
  if (array.length === 0) {
    return Q.resolve([]);
  }
  ...
  var callNext = function(index) {
    ...
    Q.resolve(promiseReply).then(function(reply) {
      replies[index] = reply;
