...
helpers.display = undefined;
helpers.options = undefined;

helpers.setup = function(options) {
  helpers.options = options;
  helpers.display = options.display;
};
...
helpers._actual = {
  input: function() {
    return helpers.display.inputter.element.value;
  },

  hints: function() {
    var templateData = helpers.display.completer._getCompleterTemplateData();
  ...
  markup: function() {
    var cursor = helpers.display.inputter.element.selectionStart;
    var statusMarkup = helpers.display.requisition.getInputStatusMarkup(cursor);
  ...
  cursor: function() {
    return helpers.display.inputter.element.selectionStart;
  },

  current: function() {
    return helpers.display.requisition.getAssignmentAt(helpers._actual.cursor()).param.name;
  },

  status: function() {
    return helpers.display.requisition.getStatus().toString();
  },

  predictions: function() {
    var cursor = helpers.display.inputter.element.selectionStart;
    var assignment = helpers.display.requisition.getAssignmentAt(cursor);
  ...
  unassigned: function() {
    return helpers.display.requisition._unassigned.map(function(assignment) {
      return assignment.arg.toString();
    }.bind(this));
  },

  outputState: function() {
    var outputData = helpers.display.focusManager._shouldShowOutput();
    return outputData.visible + ':' + outputData.reason;
  },

  tooltipState: function() {
    var tooltipData = helpers.display.focusManager._shouldShowTooltip();
    return tooltipData.visible + ':' + tooltipData.reason;
  },

  options: function() {
    return helpers.display.tooltip.field.menu.items.map(function(item) {
      return item.name.textContent ? item.name.textContent : item.name;
    });
  },

  error: function() {
    return helpers.display.tooltip.errorEle.textContent;
  }
};
...
helpers._createDebugCheck = function() {
  var requisition = helpers.display.requisition;
...
helpers.focusInput = function() {
  helpers.display.inputter.focus();
};
...
helpers.pressKey = function(keyCode) {
  ...
  helpers.display.inputter.onKeyDown(fakeEvent);
  return helpers.display.inputter.handleKeyUp(fakeEvent);
};
...
helpers.setInput = function(typed, cursor) {
  ...
  var inputter = helpers.display.inputter;
  ...
  return promise.then(function() {
    if (cursor != null) {
      helpers.display.inputter.setCursor({ start: cursor, end: cursor });
    }
    else {
      ...
      if (helpers.options.isJsdom) {
        helpers.display.inputter.setCursor({
          start: typed.length,
          end: typed.length
        });
      }
...
helpers._check = function(name, checks) {
  ...
  if ('tooltipState' in checks) {
    if (helpers.options.isJsdom) {
      assert.skip('Skipped ' + name + '/tooltipState due to jsdom');
    }
    else {
  ...
  if ('outputState' in checks) {
    if (helpers.options.isJsdom) {
      assert.skip('Skipped ' + name + '/outputState due to jsdom');
    }
    else {
  ...
  if (checks.args != null) {
    ...
    Object.keys(checks.args).forEach(function(paramName) {
      ...
      if ('name' in check) {
        if (helpers.options.isJsdom) {
          assert.skip('Skipped arg.' + paramName + '.name due to jsdom');
        }
        else {
...
helpers._exec = function(name, expected) {
  ...
  var output = helpers.display.requisition.exec({ hidden: true });
  ...
  if (!helpers.options.window.document.createElement) {
    assert.skip('skipping output tests (missing doc.createElement) for ' + name);
    return Promise.resolve();
  }
  ...
  var checkOutput = function() {
    var div = helpers.options.window.document.createElement('div');
    output.toDom(div);
...
/**
 * Some helpers to allow us to skip tests based on certain pre-defined
 * tests.
 */
helpers.reason = {
  jsdom: function jsdom() {
    return helpers.options.isJsdom;
  },
  phantomjs: function phantomjs() {
    return helpers.options.isPhantomjs;
  },
  firefox: function firefox() {
    return helpers.options.isFirefox;
  },
  createComandCheck: function(commandName) {
    return function commandMissing() {
      return canon.getCommand(commandName) == null;
    };
  }
};
...
helpers.audit = function(audits) {
  ...
  return util.promiseEach(audits, function(audit) {
    ...
    if (audit.skipIf && audit.skipIf(helpers.options)) {
      var reason = audit.skipIf.name ? 'due to ' + audit.skipIf.name : '';
      assert.skip('Skipped ' + name + ' ' + reason);
      return Promise.resolve(undefined);
    }

    if (audit.skipRemainingIf && audit.skipRemainingIf(helpers.options)) {
      skipReason = audit.skipRemainingIf.name ?
          'due to ' + audit.skipRemainingIf.name :
          '';
      assert.skip('Skipped ' + name + ' ' + skipReason);
      return Promise.resolve(undefined);
    }
