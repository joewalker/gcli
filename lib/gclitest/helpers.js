...
helpers._actual = {
  input: function(options) {
    return options.display.inputter.getInputState().typed;
  },
  ...
  cursor: function(options) {
    return options.display.inputter.getInputState().cursor.start;
  },
    ...
    inputPromise = inputter.setInput('').then(function() {
      return util.promiseEach(chunks, function(chunk) {
        if (chunk.charAt(0) === '<') {
          ...
          return inputter.setInput(inputter.getInputState().typed + chunk);
        }
      });
    });
  }
