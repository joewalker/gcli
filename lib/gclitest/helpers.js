 ...
 *
 * The options used by withTab include:
 ...
 *
 * Normally withTab will create an options object containing the values as
...
helpers.withTab = function(url, callback, options) {
  ...
  var onPageLoad = function() {
    ...
    options.window = options.browser.contentWindow;
 ...
 * Warning: For use with Firefox Mochitests only.
 *
 * As withTab, but that also opens the developer toolbar. In addition a new
 * 'display' property is added to the options object with the display from GCLI
 * in the developer toolbar
 */
helpers.withToolbarTab = function(url, callback, options) {
  return helpers.withTab(url, function(innerOptions) {
...
helpers._createDebugCheck = function(options) {
  ...
  return util.all(hintsPromise, predictionsPromise).then(function(values) {
    ...
    output += '      error: \'' + helpers._actual.error(options) + '\',\n';
...
helpers._check = function(options, name, checks) {
  ...
  if ('error' in checks) {
    assert.is(helpers._actual.error(options), checks.error, 'error' + suffix);
  }
...
helpers.audit = function(options, audits) {
  ...
  return util.promiseEach(audits, function(audit) {
    ...
    if (audit.skipRemainingIf && audit.skipRemainingIf(options)) {
