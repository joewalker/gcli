...
helpers._actual = {
  ...
  hints: function() {
    ...
    // We need to resolve directTabText, emptyParameters and arrowTabText
    // which promised() is ideal for, but it's broken for array parameters
    /*
    ...
    */
    ...
    // So we resolve the promises individually
    var deferred = Q.defer();
    Q.resolve(templateData.directTabText).then(function(directTabText) {
      Q.resolve(templateData.emptyParameters).then(function(emptyParameters) {
        Q.resolve(templateData.arrowTabText).then(function(arrowTabText) {
          var combined = join(directTabText, emptyParameters, arrowTabText);
...
helpers._createDebugCheck = function() {
  ...
  var hints = helpers._actual.hints();
  if (typeof hints.then === 'function') {
    hints = '<promise>';
  }
...
helpers.check = function(checks) {
  ...
  if ('hints' in checks) {
    var checkHints = test.checkCalled(function(actualHints) {
      test.is(actualHints, checks.hints, 'hints');
    });
...
helpers.exec = function(options, tests) {
  ...
  var checkOutput = test.checkCalled(function() {
