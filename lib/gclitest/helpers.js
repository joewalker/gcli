...
helpers.audit = function(audits) {
  var auditNext = function(index) {
    ...
    Q.resolve(setupDone).then(function() {
      helpers._check(name, audit.check).then(function() {
        var complete = function() {
          var postDone = undefined;
          if (typeof audit.post === 'function') {
            postDone = audit.post();
          }

          Q.resolve(postDone).then(function() {
        ...
        if (audit.exec != null) {
          helpers._checkExec(name, audit.exec).then(function() {
            complete();
    ...
    }.bind(this), console.error).then(null, console.error);
  }.bind(this);
