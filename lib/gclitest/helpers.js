...
helpers._actual = {
  input: function(options) {
    return options.display.terminal.getInputState().typed;
  },

  hints: function(options) {
    var terminal = options.display.terminal;
    return terminal._getCompleterTemplateData().then(function(data) {
  ...
  cursor: function(options) {
    return options.display.terminal.getInputState().cursor.start;
  },

  current: function(options) {
    var cursor = helpers._actual.cursor(options);
    return options.display.requisition.getAssignmentAt(cursor).param.name;
  },
  ...
  outputState: function(options) {
    var outputData = options.display.terminal.focusManager._shouldShowOutput();
    return outputData.visible + ':' + outputData.reason;
  },

  tooltipState: function(options) {
    var tooltipData = options.display.terminal.focusManager._shouldShowTooltip();
    return tooltipData.visible + ':' + tooltipData.reason;
  },

  options: function(options) {
    if (options.display.terminal.field.menu == null) {
      return [];
    }
    return options.display.terminal.field.menu.items.map(function(item) {
      return item.name.textContent ? item.name.textContent : item.name;
    });
  },

  message: function(options) {
    return options.display.terminal.errorEle.textContent;
  }
};
...
helpers.focusInput = function(options) {
  checkOptions(options);
  options.display.terminal.focus();
};
...
helpers.pressKey = function(options, keyCode) {
  ...
  options.display.terminal.onKeyDown(fakeEvent);
  return options.display.terminal.handleKeyUp(fakeEvent);
};
...
helpers.setInput = function(options, typed, cursor) {
  ...
  if (typed.indexOf('<') === -1) {
    inputPromise = terminal.setInput(typed);
  }
  else {
  ...
  return inputPromise.then(function() {
    ...
    options.display.terminal.focusManager.onInputChange();
...
helpers.audit = function(options, audits) {
  ...
  return util.promiseEach(audits, function(audit) {
    ...
    return options.display.terminal.setInput('');
  });
};
