...
helpers.check = function(checks) {
  ...
  if ('hints' in checks) {
    var hintCheck = test.checkCalled(function(actualHints) {
      assert.is(actualHints, checks.hints, 'hints');
    });
    Q.resolve(helpers._actual.hints()).then(hintCheck, console.error);
  }

  if ('predictions' in checks) {
    var predictionsCheck = test.checkCalled(function(actualPredictions) {
      helpers._arrayIs(actualPredictions, checks.predictions, 'predictions');
    });
    Q.resolve(helpers._actual.predictions()).then(predictionsCheck, console.error);
  }

  if ('predictionsContains' in checks) {
    var containsCheck = test.checkCalled(function(actualPredictions) {
    ...
    Q.resolve(helpers._actual.predictions()).then(containsCheck, console.error);
  }
