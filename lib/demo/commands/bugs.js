...
var bugzCommandSpec = {
  ...
  exec: function(args, context) {
    ...
    queryBugzilla(query, function(json) {
      ...
      var div = util.dom.createElement(doc, 'div');

      var p = util.dom.createElement(doc, 'p');
      p.appendChild(doc.createTextNode('Open GCLI meta-bugs (i.e. '));
      var a = util.dom.createElement(doc, 'a');
      ...
      json.bugs.forEach(function(bug) {
        ...
        queryBugzilla('blocked=' + bug.id, function(json) {
          var subul = util.dom.createElement(doc, 'ul');
          json.bugs.forEach(function(bug) {
      ...
      var table = util.dom.createElement(doc, 'table');
      var header = util.dom.createElement(doc, 'tr');
      header.innerHTML = '<th>Days</th><th>Best</th><th>Likely</th><th>Worst</th>';
      table.appendChild(header);
      div.appendChild(table);

      var compRow = util.dom.createElement(doc, 'tr');
      var completed = util.dom.createElement(doc, 'td');
      completed.innerHTML = 'Completed';
      compRow.appendChild(completed);
      predictions.completed.bestTd = util.dom.createElement(doc, 'td');
      compRow.appendChild(predictions.completed.bestTd);
      predictions.completed.likelyTd = util.dom.createElement(doc, 'td');
      compRow.appendChild(predictions.completed.likelyTd);
      predictions.completed.worstTd = util.dom.createElement(doc, 'td');
      compRow.appendChild(predictions.completed.worstTd);
      table.appendChild(compRow);

      var outstRow = util.dom.createElement(doc, 'tr');
      var outstanding = util.dom.createElement(doc, 'td');
      outstanding.innerHTML = 'Outstanding';
      outstRow.appendChild(outstanding);
      predictions.outstanding.bestTd = util.dom.createElement(doc, 'td');
      outstRow.appendChild(predictions.outstanding.bestTd);
      predictions.outstanding.likelyTd = util.dom.createElement(doc, 'td');
      outstRow.appendChild(predictions.outstanding.likelyTd);
      predictions.outstanding.worstTd = util.dom.createElement(doc, 'td');
      outstRow.appendChild(predictions.outstanding.worstTd);
      table.appendChild(outstRow);

      predictions.summary = util.dom.createElement(doc, 'p');
...
function liFromBug(doc, bug, predictions) {
  ...
  var li = util.dom.createElement(doc, 'li');
  if (done) {
  ...
  if (bug.whiteboard.indexOf('[minotaur]') !== -1) {
    ...
    var minsum = util.dom.createElement(doc, 'span');
