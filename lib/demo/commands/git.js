...
var gitRemote = {
  ...
  manual: 'Manage the set of repositories ("remotes") whose branches you track.',
};
  ...
  params: [
    {
      name: 'name',
      type: 'string',
      description: 'A short name for a remote repository'
    },
    {
      name: 'url',
      type: 'string',
      description: 'URL that declares where the remote repository is'
    }
  ],
  exec: function(args, context) {
    return "This is only a demo of UI generation.";
...
exports.startup = function(data, reason) {
  gcli.addType(commitObject);
  gcli.addType(existingFile);

  gcli.addCommand(git);
  gcli.addCommand(gitAdd);
  gcli.addCommand(gitCommit);
  gcli.addCommand(gitRemote);
  gcli.addCommand(gitRemoteRename);
  gcli.addCommand(gitRemoteAdd);
};
...
exports.shutdown = function(data, reason) {
  ...
  gcli.removeCommand(gitRemote);
  gcli.removeCommand(gitRemoteRename);
  gcli.removeCommand(gitRemoteAdd);

  gcli.removeType(commitObject);
  gcli.removeType(existingFile);
};
