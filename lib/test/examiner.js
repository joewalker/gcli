...
examiner.run = function(options, callback) {
  ...
  var doneSetup = callAll(examiner.setups, [ options ]);

  var doneTests = doneSetup.then(function() {
  ...
  return doneTests.then(function() {
    return callAll(examiner.shutdowns, [ options ]);
  });
};

function callAll(funcs, args) {
  return util.promiseEach(funcs, function(func) {
...
/**
 * Internal function to run all the tests in this suite asynchronously without
 * any setup/shutdown at the suite level. i.e. this function is designed to be
 * called by examiner.run or suite.run.
 */
Suite.prototype._run = function(options) {
  this._setup(options);
