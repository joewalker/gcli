 ...
 * depend on util/util.createEvent (which could make matters worse) so we just
...
exports.checkCalled = function(func, scope) {
  ...
  if (currentTest == null) {
    throw new Error('No current test in checkCalled setup');
  }

  var todo = function() {
    // It's not inconceivable that doing some action fires off a list of
    // unrelated events, or in other words one test could cause some delayed
    // behaviour in another test to run, so reset the current test
    var oldCurrent = exports.currentTest;
    exports.currentTest = currentTest;

    if (exports.currentTest.status !== stati.executing) {
      console.error('Post finish result from ' + exports.currentTest);
    }
    ...
    try {
      ...
      exports.currentTest = oldCurrent;
