...
Assignment.prototype = {
    ...
    getHint: function() {
        ...
        if (this.param.description) {
            ...
            message += '<strong>' + desc + '</strong>: ';
        }
        else {
            message += '<strong>' + this.param.name + '</strong>: ';
        }
     ...
     */
    isPositionCaptured: function(position, endIsPrev) {
        if (!this.arg) {
            return false;
        }
...
var commandParam = {
    ...
    getCustomHint: function(command, arg) {
        ...
        // If there are required parameters, we're not done, and the problem is
        // at the cursor
        if (hasRequired) {
            return new Hint(Status.INCOMPLETE, docs.join(''),
                Argument.AT_CURSOR, Argument.AT_CURSOR, arg.predictions);
        }

        // If this is a parent of a sub-command, we're not done, and the
        // problem is with the command parameter
        if (!command.exec) {
            return new Hint(Status.INCOMPLETE, docs.join(''), arg);
        }
...
(function() {
     ...
     * cursor up/down is pressed, then we work on the existing parameter,
     * however for hint purposes we may say were on the next parameter.
     * @see Assignment.prototype.isPositionCaptured
     */
    CliRequisition.prototype.getAssignmentAt = function(position, endIsPrev) {
        ...
        for (var i = 0; i < assignments.length; i++) {
            ...
            if (assignment.isPositionCaptured(position, endIsPrev)) {
                return assignment;
            }
        }
