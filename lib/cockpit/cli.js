...
var Conversion = require('cockpit/types').Conversion;
var ArrayType = require('cockpit/types').ArrayType;
var StringType = require('cockpit/types').StringType;
var BooleanType = require('cockpit/types').BooleanType;
var SelectionType = require('cockpit/types').SelectionType;
...
Assignment.prototype = {
    ...
    setConversion: function(conversion) {
        ...
        if (oldConversion != null &&
                this.conversion.valueEquals(oldConversion)) {
            return;
        }
     ...
     */
    ensureVisibleArgument: function() {
...
Requisition.prototype.exec = function(input) {
    if (input) {
        ...
        // means there is no command line to parse, and leave 'typed' off the
        // param list so canon.exec can do the same
        if (input.args != null) {
            var command = canon.getCommand(input.typed);
            return canon.exec(command, this.env, input.args);
        }
        else {
            this.update(input);
        }
    ...
    return canon.exec(this.commandAssignment.getValue(),
          this.env,
          this.getArgsObject(),
