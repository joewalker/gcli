 ...
 */
Requisition.prototype.getParameterNames = function(options) {
    if (options) {
        var names = [];
        Object.keys(this._assignments).forEach(function(name) {
            var posnl = this._assignments[name].param.isPositionalAllowed();
            if ((options.positionalOnly && posnl) ||
                    (options.nonPositionalOnly && !posnl)) {
                names.push(name);
            }
        }, this);
        return names;
...
Requisition.prototype.getInputStatusMarkup = function() {
    ...
    for (var i = 0; i < argTraces.length; i++) {
        ...
        if (argTrace.part === 'text') {
            status = argTrace.arg.assignment.getStatus(argTrace.arg);
