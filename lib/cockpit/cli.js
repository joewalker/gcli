...
Assignment.prototype = {
     ...
     * <p>Important! Use setConversion() to mutate
     */
    value: undefined,
    ...
    complete: function() {
        if (this.conversion && this.conversion.predictions &&
            ...
            var arg = this.arg.beget(this.param.type.stringify(value));
    ...
    decrement: function() {
        ...
        if (replacement != null) {
            var arg = this.arg.beget(this.param.type.stringify(replacement));
    ...
    shiftArgument: function(amount) {
        this.arg = this.arg.begetShifted(amount);
    },
...
Requisition.prototype._onAssignmentChange = function(ev) {
    ...
    for (var i = 0; i < this._args.length; i++) {
        if (this._args[i] === ev.oldArg) {
            this._args[i] = ev.newArg;
            updated = true;
        }
    }
    ...
    // TODO: Again, with named arguments we'll need to do better
    if (!updated) {
        this._args.push(ev.newArg);
    }
...
Requisition.prototype._onCommandAssignmentChange = function(ev) {
    ...
    if (command) {
        this._args = [ this.commandAssignment.arg ];
...
Requisition.prototype.setDefaultArguments = function() {
    this.getAssignments().forEach(function(assignment) {
        var conversion = assignment.param.type.parse(new Argument());
...
Requisition.prototype.getAssignmentAt = function(cursor) {
    ...
    for (i = 0; i < this._args.length; i++) {
        ...
        var next = this._args.length > i + 1 ? this._args[i + 1] : arg;
    ...
    return argForPos[cursor - 1].assignment;
 ...
 */
 function _isPositionCaptured(assignment, cursor) {
    ...
    // Note we don't check if position >= assignment.arg.start because that's
...
Requisition.prototype._split = function(args) {
    var argsUsed = 1;
    var conversion;
    var oldArg = this.commandAssignment.arg;
    ...
    if (!Argument.equals(oldArg, conversion.arg)) {
...
Requisition.prototype._assign = function(args) {
    ...
    if (this.assignmentCount === 1) {
        ...
        if (assignment.param.type.name === 'text') {
            ...
            if (!Argument.equals(assignment.arg, arg)) {
    ...
    names.forEach(function(name) {
        ...
        if (assignment.param.type instanceof ArrayType) {
            ...
            Array.prototype.push.call(arrayArg, args);
        }
        else {
