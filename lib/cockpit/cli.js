...
Argument.prototype = {
    ...
    merge: function(following) {
        if (following.emitter != this.emitter) {
            throw new Error('merged Arguments must share EventEmitters');
        }
        return new Argument(
            this.emitter,
 ...
 * @constructor
 */
function Assignment(param, paramIndex, requisition) {
    this.param = param;
    this.paramIndex = paramIndex;
...
Assignment.prototype = {
     ...
     *
     * @param {number} position The cursor position to query
     ...
     */
    _isPositionCaptured: function(position, endIsPrev) {
        if (!this.arg) {
            return false;
        }
...
function Requisition(env) {
    this.env = env;
    this.commandAssignment = new Assignment(commandParam, 0, this);

    // We populate assignments with noArgument when there is nothing provided
    // by the user because we need to collect TAB completions (etc) an update
    // the command line
    this.noArgument = new Argument(this, '', Argument.AT_CURSOR,
        Argument.AT_CURSOR, '', '');
}
Requisition.prototype = {
    ...
    _assignmentChanged: function(assignment) {
        ...
        if (assignment.value) {
            for (var i = 0; i < assignment.value.params.length; i++) {
                var param = assignment.value.params[i];
                this._assignments[param.name] = new Assignment(param, i, this);
            }
        }
    ...
    getWorstHint: function() {
        Hint.sort(this._hints, Hint.sort.Order.STATUS_PARAM_DISTANCE);
    ...
    setDefaultValues: function() {
        this.getAssignments().forEach(function(assignment) {
            assignment.setArgument(this.noArgument);
        }, this);
    },
...
(function() {
     ...
     */
    CliRequisition.prototype.getAssignmentAt = function(position, endIsPrev) {
        ...
        for (var i = 0; i < assignments.length; i++) {
            ...
            if (assignment._isPositionCaptured(position, endIsPrev)) {
                return assignment;
            }
        }
    ...
    CliRequisition.prototype._split = function(args) {
        ...
        while (argsUsed <= args.length) {
            ...
            if (!this.commandAssignment.value  ||
                    this.commandAssignment.value.exec) {
                break;
            }
    ...
    CliRequisition.prototype._assign = function(args) {
        ...
        if (this.assignmentCount === 0) {
            this.unassigned = Argument.merge(args);
            this._hints.push(new Hint(Status.ERROR,
