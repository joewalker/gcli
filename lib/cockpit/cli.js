...
Argument.prototype = {
     ...
     */
    beget: function(replText, replPrefix, replSuffix) {
        ...
        if (replPrefix != null) {
            prefix = ' ' + replPrefix;
            start = start - this.prefix.length + replPrefix.length;
        }
        ...
        if (replSuffix != null) {
            suffix = replSuffix;
            end = end - this.suffix.length + replSuffix.length;
        }
...
Assignment.prototype = {
    ...
    setArgument: function(newArg) {
        ...
        this.conversion = this.param.type.parse(newArg.text);
