 ...
 * TODO: What happens when we have escaped characters? They should be unescaped
 * in the text part, but can they be reliably re-escaped in toString()?
 * @constructor
 */
...
(function() {
    ...
    CliRequisition.prototype.getAssignmentAt = function(position) {
        ...
        // Assume this is part of the last assignment
        return this.getAssignmentAt(names.length - 1);
    };
            ...
            if (i >= typed.length) {
                ...
                if (mode !== OUTSIDE) {
                    ...
                    if (i !== start) {
                        ...
                        args.push(new Argument(this, '', i, i, prefix, ''));
                    }
                }
                break;
            }
