...
var NamedArgument = require('cockpit/argument').NamedArgument;
var BooleanNamedArgument = require('cockpit/argument').BooleanNamedArgument;
var MergedArgument = require('cockpit/argument').MergedArgument;
...
exports.startup = function(data, reason) {
    canon.upgradeParam(commandParam, 'command');
};
...
function Assignment(param, paramIndex) {
    ...
    this._conversion = this.param.getDefault ?
...
Assignment.prototype = {
    ...
    setValue: function(value) {
        ...
        if (value === undefined) {
            ...
            this.arg = this.arg.beget(text, {
                ...
                quote: text.indexOf(' ') >= 0
            });
        }

        ev.newValue = this.value;
        ev.newArg = this.arg;
    ...
    setArgument: function(newArg) {
        ...
        this._conversion = this.param.type.parse(newArg.text);
        this.value = this._conversion.value;
    ...
    getStatus: function() {
        ...
        return this._conversion ? this._conversion.status : Status.VALID;
    },

    getMessage: function() {
        if (this._conversion && this._conversion.message) {
            return this._conversion.message;
        }
        return '';
    },

    getPredictions: function() {
        if (this._conversion && this._conversion.predictions) {
            return this._conversion.predictions;
        }
        return [];
    },
    ...
    complete: function() {
        if (this._conversion && this._conversion.predictions &&
                this._conversion.predictions.length > 0) {
            this.setValue(this._conversion.predictions[0]);
        }
    },
    ...
    _isPositionCaptured: function(cursor, endIsPrev) {
        ...
        if (cursor === this.arg.end) {
            ...
            return this._conversion.status !== Status.VALID ||
                    this._conversion.predictions.length !== 0;
        }
        ...
        while (i >= args.length) {
            if (assignment.param.isNamedParam(args[i].text)) {
                ...
                if (assignment.param.type.name === 'boolean') {
                    assignment.setArgument(new BooleanNamedArgument(arg));
                }
                else {
                    var valueArg = null;
                    if (i + 1 >= args.length) {
                    ...
                    assignment.setArgument(new NamedArgument(arg, valueArg));
                }
            }
            else {
