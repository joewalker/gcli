...
Assignment.prototype = {
    ...
    getHint: function() {
        ...
        if (this.param.name === commandParam.name && this.value && this.arg) {
            var status = this.value.exec ? Status.VALID : Status.INCOMPLETE;
            return new Hint(status, '', 0, this.arg);
        }
    ...
    getStatus: function() {
        ...
        // Incomplete if this is a commandParam with an in-executable command
        if (this.param.name === commandParam.name && this.value && !this.value.exec) {
...
(function() {
    ...
    CliRequisition.prototype.getInputStatusMarkup = function() {
        ...
        // Incomplete hints are errors unless selection/argument overlap
        ...
        function updateStatuses(status, arg) {
            var start = arg ? arg.start : Argument.AT_CURSOR;
            var end = arg ? arg.end : Argument.AT_CURSOR;
            var startInHint = c.start >= start && c.start <= end;
            var endInHint = c.end >= start && c.end <= end;
            var inHint = startInHint || endInHint;
            if (!inHint && start !== 0 && status === Status.INCOMPLETE) {
                status = Status.ERROR;
            }
            for (var i = start; i < end; i++) {
        ...
        this.getAssignments(true).forEach(function(assignment) {
            ...
            updateStatuses(status, assignment.arg);
        }, this);

        if (this.unassigned) {
            updateStatuses(Status.ERROR, this.unassigned);
        }
