...
define(function(require, assert, module) {
...
assert.test = {
  ...
  assertEqual: function(expected, actual) {
    if (!assert.test._isEqual(expected, actual)) {
      assert.test._recordThrow("assertEqual", arguments);
    }
  },

  verifyEqual: function(expected, actual) {
    if (!assert.test._isEqual(expected, actual)) {
      assert.test._recordTrace("verifyEqual", arguments);
    }
  },

  assertNotEqual: function(expected, actual) {
    if (assert.test._isEqual(expected, actual)) {
      assert.test._recordThrow("assertNotEqual", arguments);
    }
  },

  verifyNotEqual: function(expected, actual) {
    if (assert.test._isEqual(expected, actual)) {
      assert.test._recordTrace("verifyNotEqual", arguments);
    }
  },
  ...
  _isEqual: function(expected, actual, depth) {
    ...
    if (expected instanceof Array) {
      ...
      for (var i = 0; i < actual.length; i++) {
        var inner = assert.test._isEqual(actual[i], expected[i], depth + 1);
