...
var tsa = {
    ...
    params: [
        ...
        { name: 'arr', type: { name: 'array', subtype: 'text' } },
    ],
    exec: function(env, args, request) {}
};
...
var tsm = {
    name: 'tsm',
    description: 'a 3-param test selection|text|number',
    params: [
...
exports.testSplit = function() {
    ...
    test.verifyEqual('s', requ.commandAssignment.arg.text);

    args = requ._tokenize('tsv');
    requ._split(args);
    test.verifyEqual([], args);
    test.verifyEqual('tsv', requ.commandAssignment.value.name);
...
exports.testCli = function() {
    ...
    function update(input) {
        ...
        assignC = requ.getAssignmentAt(input.cursor.start, true);
    ...
    test.verifyEqual(null, requ.commandAssignment.value);
    ...
    test.verifyNull(requ.commandAssignment.value);
    ...
    test.verifyEqual('fred', requ.commandAssignment.arg.text);
    test.verifyEqual(null, requ._unassigned);
    ...
    test.verifyEqual('fred', requ.commandAssignment.arg.text);
    test.verifyEqual('one', requ._unassigned.text);

    update({ typed:  'tsr', cursor: { start: 3, end: 3 } });
    test.verifyEqual('VVV', statuses);
    test.verifyEqual('tsr', requ.commandAssignment.value.name);
    ...
    test.verifyEqual('VVV', statuses);
    test.verifyEqual(Status.INCOMPLETE, status);
    test.verifyEqual('tsu', requ.commandAssignment.value.name);
    //test.verifyEqual(undefined, assign1.arg);
    test.verifyEqual(undefined, assign1.value);
    ...
    test.verifyEqual(Status.INCOMPLETE, status);
    test.verifyEqual('tsn', requ.commandAssignment.value.name);
    ...
    test.verifyEqual(Status.ERROR, status);
    test.verifyEqual('tsn x', requ.commandAssignment.arg.text);
    test.verifyEqual(undefined, assign1);

    update({ typed:  'tsn dif', cursor: { start: 7, end: 7 } });
    test.verifyEqual('VVVVVVV', statuses);
    test.verifyEqual(Status.INCOMPLETE, status);
    test.verifyEqual('tsn dif', requ.commandAssignment.value.name);
    //test.verifyEqual(undefined, assign1.arg);
    //test.verifyEqual(undefined, assign1.value);
    ...
    test.verifyEqual('VVVVVVV', statuses);
    test.verifyEqual(Status.VALID, status);
    test.verifyEqual('tsn ext', requ.commandAssignment.value.name);
    //test.verifyEqual(undefined, assign1.arg);
    //test.verifyEqual(undefined, assign1.value);
