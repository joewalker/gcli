...
function imageUrl(path) {
    try {
        return require('text!cockpit/ui/' + path);
    }
    catch (ex) {
...
function RequestView(request) {
    ...
    if (!RequestView._row) {
        ...
        RequestView._row = templates.querySelector('.cptRow');
    }
...
RequestView.startup = function(options) {
    ...
    RequestView.requests = options.requests;

    canon.addEventListener('output',  function(ev) {
        if (!ev.request.view) {
            var view = RequestView.create(ev.request);
            ev.request.view = view;
        }
        ev.request.view.onRequestChange(ev);
    });
};

RequestView.create = function(request) {
...
RequestView.prototype = {
    ...
    copyToInput: function() {
        if (RequestView.cliView) {
            RequestView.cliView.setInput(this.request.typed);
        }
    },
    ...
    onRequestChange: function(ev) {
        ...
        var node;
        if (typeof this.request.output === 'string') {
            node = dom.createElement('p', null, RequestView.doc);
            dom.setInnerHtml(node, this.request.output);
        } else {
            node = this.request.output;
        }
