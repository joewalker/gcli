...
(function() {
    ...
    reqTempl = templates.querySelector('.cptRequisition');
})();
...
CliView.prototype = {
    ...
    update: function() {
        ...
        if (this.cli.commandAssignment.value) {
            message += this.commandDialog(this.cli);
            this.hinter.innerHTML = '';
            this.hinter.appendChild(this.reqEle);
        }
        ...
        if (predictions && predictions.length > 0) {
            message += '<div class="cptOptions">';
            predictions.forEach(function(prediction) {
                message += '<div class="cptOption">';
                message += '  <span class="cptOptName">';
                message +=   (prediction.name ? prediction.name : prediction);
                message += '  </span>';
                message += '  <span class="cptOptDesc">';
                message +=   (prediction.description ? prediction.description : '');
                message += '  </span>';
                message += '</div>';
            }, this);
            message += '</div>';
        ...
        this.messageEle.innerHTML = message;
        if (false/*hint.length === 0*/) {
    ...
    onArgChange: function(ev) {
        ...
        var argStart = ev.argument.start;
        var argEnd = ev.argument.end;
        ...
        var inserted = typeof ev.text === 'string' ? ev.text : ev.text.name;
    ...
    commandDialog: function(cli) {
        var command = cli.commandAssignment.value;

        new Templater().processNode(reqTempl.cloneNode(true), this);

        return this.reqEle;

        var docs = [];
        docs.push(command.getDescription());
        docs.push('<br/>');
        ...
        */
        ...
        if (command.params.length > 0) {
            ...
            command.params.forEach(function(param) {
                var assignment = this.cli.getAssignment(param.name);
                docs.push('<div>');
                docs.push('<strong>' + param.name + '</strong>: ');
                docs.push('<input type="text" value="' + assignment.arg.text + '">');
                ...
                docs.push('<br/>');
                docs.push('<small>');
                docs.push(param.description || '(No description)');
                docs.push('</small>');
                docs.push('</div>');
            }, this);
            docs.push('</div>');
