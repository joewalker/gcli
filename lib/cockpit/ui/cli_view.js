...
var ArgFetcher = require('cockpit/ui/arg_fetch').ArgFetcher;
 ...
 */
function CliView(env) {
    this.requ = new Requisition(env);
    this.doc = document;
    this.win = dom.getParentWindow(this.doc);
    this.env = env;
    this.argFetcher = new ArgFetcher(this.doc, this.requ);

    // TODO: we should have a better way to specify command lines???
    this.element = this.doc.getElementById('cockpitInput');
    if (!this.element) {
        console.log('No element with an id of cockpit. Skipping CliView');
        return;
    }
...
CliView.prototype = {
    ...
    createElements: function() {
        ...
        this.hintParent = this.doc.createElement('div');
        this.hintParent.className = 'cptHintParent';
