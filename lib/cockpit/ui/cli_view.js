...
SelectionField.prototype.createElement = function(assignment) {
    ...
    this.onChange = function() {
        if (this.select.value === SelectionField.DEFAULT_VALUE) {
            this.assignment.setValue(null);
        }
        else {
            this.assignment.setValue(this.opts[this.select.value]);
        }
    }.bind(this);
    this.select.addEventListener('change', this.onChange, false);

    this.onAsmtChange = function() {
        var value = this.assignment.arg.text;
        if (!this.opts[value]) {
            value = SelectionField.DEFAULT_VALUE;
        }
