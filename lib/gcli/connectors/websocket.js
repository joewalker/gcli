    ...
    connect: function(url) {
      ...
      if (connections[url] == null) {
        connections[url] = new Promise((resolve, reject) => {
          ...
          const wsc = new WebsocketConnection(socket, url);

          // What to do when a reply is received
          socket.on('reply', wsc.onReply);
          socket.on('event', wsc.onEvent);
          socket.on('connected', () => resolve(wsc));
          socket.on('connect_error', () => reject(wsc));
        });
      }
...
WebsocketConnection.prototype.onReply = function(res) {
  const deferred = this.deferreds[res.id];
  if (deferred == null) {
...
WebsocketConnection.prototype.disconnect = function() {
  Object.keys(this.deferreds).forEach(id => {
