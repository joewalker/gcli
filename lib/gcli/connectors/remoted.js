...
Remoter.prototype.exposed = {
  ...
  parseType: method(function(typed, paramName) {
    return this.requisition.update(typed).then(() => {
      const conversion = this.requisition.getAssignment(paramName).conversion;
      return Promise.resolve(conversion.predictions).then(predictions => {
        return {
  ...
  parseFile: method(function(typed, filetype, existing, matches) {
    ...
    const context = this.requisition.executionContext;
    return fileparser.parse(context, typed, options).then(reply => {
      reply.status = reply.status.toString();
      if (reply.predictor == null) {
        return reply;
      }

      return reply.predictor().then(predictions => {
