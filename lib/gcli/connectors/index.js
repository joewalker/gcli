...
/**
 * This is the set of items that are needed in most client setups
 */
var clientItems = [
...
exports.createRemoteTerminal = function(options) {
  ...
  gcli.addItems(requiredConverters.filter(function(item) {
    return item.item === 'converter';
  }));

  var connector = connectors.get('websocket');
  return connector.connect(options.url).then(function(connection) {
    ...
    return connection.call('specs').then(function(specs) {
      exports.addItems(gcli, specs, connection);
      return exports.enableMocks(connection).then(function() {
...
exports.enableMocks = function(connection) {
  return connection.call('execute', { typed: 'mocks on' }).then(function(data) {
    if (data.error) {
