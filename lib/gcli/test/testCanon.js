...
const commandsChange = function(ev) {
  events++;
};
...
exports.testAddRemove1 = function(options) {
  const commands = options.requisition.system.commands;
...
exports.testAltCommands = function(options) {
  const commands = options.requisition.system.commands;
  const altCommands = new Commands(options.requisition.system.types);
  ...
  const commandSpecs = altCommands.getCommandSpecs();
  ...
  const customProps = [ 'customProp1', 'customProp2', 'customProp3', ];
  const commandSpecs2 = altCommands.getCommandSpecs(customProps);
