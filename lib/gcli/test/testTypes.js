...
exports.testGetSpec = function(options) {
  return forEachType(options, {}, function(type) {
    ...
    // Things that extend selection or delegate types generally don't have
    // a new name
    if (!type.isSelection && !type.isDelegate) {
      assert.is(example.name, type.name, 'similar spec for ' + type.name);
    }
  });
};
