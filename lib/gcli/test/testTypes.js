...
function forEachType(options, templateTypeSpec, callback) {
  ...
  return util.promiseEach(types.getTypeNames(), function(name) {
    ...
    const type = types.createType(typeSpec);
    const reply = callback(type);
...
exports.testDefault = function(options) {
  return forEachType(options, {}, function(type) {
    ...
    const blank = type.getBlank(context).value;
...
exports.testNullDefault = function(options) {
  const context = null; // Is this test still valid with a null context?

  return forEachType(options, { defaultValue: null }, function(type) {
    const reply = type.stringify(null, context);
...
exports.testGetSpec = function(options) {
  return forEachType(options, {}, function(type) {
    ...
    const spec = type.getSpec('cmd', 'param');
    ...
    const example = options.requisition.system.types.createType(spec);
