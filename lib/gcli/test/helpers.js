...
const KeyEvent = require('../util/util').KeyEvent;
...
helpers._actual = {
  ...
  hints: function(options) {
    return options.automator.getCompleterTemplateData().then(data => {
      const emptyParams = data.emptyParameters.join('');
  ...
  markup: function(options) {
    const cursor = helpers._actual.cursor(options);
    const statusMarkup = options.requisition.getInputStatusMarkup(cursor);
    return statusMarkup.map(s => {
  ...
  predictions: function(options) {
    const cursor = helpers._actual.cursor(options);
    const assignment = options.requisition.getAssignmentAt(cursor);
    const context = options.requisition.executionContext;
    return assignment.getPredictions(context).then(predictions => {
      return predictions.map(prediction => prediction.name);
    });
  },

  unassigned: function(options) {
    return options.requisition._unassigned.map(assignment => assignment.arg.toString());
  },

  outputState: function(options) {
    const outputData = options.automator.focusManager._shouldShowOutput();
    return outputData.visible + ':' + outputData.reason;
  },

  tooltipState: function(options) {
    const tooltipData = options.automator.focusManager._shouldShowTooltip();
  ...
  options: function(options) {
    ...
    return options.automator.field.menu.items.map(item => {
...
helpers._createDebugCheck = function(options) {
  ...
  const requisition = options.requisition;
  const command = requisition.commandAssignment.value;
  const cursor = helpers._actual.cursor(options);
  const input = helpers._actual.input(options);
  const padding = new Array(input.length + 1).join(' ');

  const hintsPromise = helpers._actual.hints(options);
  const predictionsPromise = helpers._actual.predictions(options);

  return Promise.all([ hintsPromise, predictionsPromise ]).then(values => {
    const hints = values[0];
    ...
    if (command) {
      ...
      requisition.getAssignments().forEach(assignment => {
...
helpers.setInput = function(options, typed, cursor) {
  checkOptions(options);
  let inputPromise;
  const automator = options.automator;
  ...
  if (typed.indexOf('<') === -1) {
    ...
    const chunks = typed.split(CHUNKER).filter(s => s !== '');
    chunkLen = chunks.length + 1;

    // We're working on this in chunks so first clear the input
    inputPromise = automator.setInput('').then(() => {
      return util.promiseEach(chunks, chunk => {
        if (chunk.charAt(0) === '<') {
          const action = ACTIONS[chunk];
  ...
  return inputPromise.then(() => {
    ...
    if (options.isFirefox) {
      const cursorStr = (cursor == null ? '' : ', ' + cursor);
...
helpers._check = function(options, name, checks) {
  ...
  if (!options.isNode && 'predictions' in checks) {
    const predictionsCheck = actualPredictions => {
      helpers.arrayIs(actualPredictions, checks.predictions, 'predictions' + suffix);
    };
  ...
  if (!options.isNode && 'predictionsContains' in checks) {
    const containsCheck = actualPredictions => {
      checks.predictionsContains.forEach(prediction => {
        const index = actualPredictions.indexOf(prediction);
        assert.ok(index !== -1,
  ...
  */
  ...
  if (checks.args != null) {
    Object.keys(checks.args).forEach(paramName => {
      const check = checks.args[paramName];
...
helpers._exec = function(options, name, expected) {
  ...
  const origLogErrors = cli.logErrors;
  if (expected.error) {
    cli.logErrors = false;
  }
  ...
  try {
    return requisition.exec({ hidden: true }).then(output => {
      ...
      if (options.isNode) {
        ...
        convertPromise = output.convert('dom', context).then(node => {
          return (node == null) ? '' : node.textContent.trim();
        });
      }

      return convertPromise.then(textOutput => {
        const doTest = (match, against) => {
        ...
        if (typeof expected.output === 'string') {
          ...
          expected.output.forEach(match => doTest(match, textOutput));
        }
        else {
...
helpers.audit = function(options, audits) {
  checkOptions(options);
  let skipReason = null;
  return util.promiseEach(audits, audit => {
    let name = audit.name;
    ...
    if (audit.skipRemainingIf) {
      const skipRemainingIf = (typeof audit.skipRemainingIf === 'function') ?
    ...
    if (audit.skipIf) {
      const skip = (typeof audit.skipIf === 'function') ?
          audit.skipIf(options) :
          !!audit.skipIf;
      if (skip) {
        const reason = audit.skipIf.name ? 'due to ' + audit.skipIf.name : '';
    ...
    const setupDone = helpers._setup(options, name, audit);
    return setupDone.then(chunkLen => {
      ...
      if (assert.currentTest) {
        const responseTime = (new Date().getTime() - startTime) / chunkLen;
      ...
      const checkDone = helpers._check(options, name, audit.check);
      return checkDone.then(() => {
        const execDone = helpers._exec(options, name, audit.exec);
        return execDone.then(data => {
          return helpers._post(name, audit, data).then(() => {
            if (assert.testLogging) {
