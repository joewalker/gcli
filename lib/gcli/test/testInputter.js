...
exports.testOutput = function(options) {
  ...
  const focusManager = terminal.focusManager;

  terminal.setInput('tss');

  const ev0 = { keyCode: KeyEvent.DOM_VK_RETURN };
  ...
  const ev1 = { keyCode: KeyEvent.DOM_VK_RETURN };
  return terminal.handleKeyUp(ev1).then(function() {
    ...
    const ev2 = { keyCode: KeyEvent.DOM_VK_F1 };
    return terminal.handleKeyUp(ev2).then(function() {
      ...
      const ev3 = { keyCode: KeyEvent.DOM_VK_ESCAPE };
