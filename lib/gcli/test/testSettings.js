...
const mockSettings = require('./mockSettings');

exports.testRegister = function(options) {
  const settings = options.requisition.system.settings;
  assert.is(settings.get('tempTBool'),
            mockSettings.tempTBool,
            'tempTBool exists before');

  settings.remove(mockSettings.tempTBoolSpec);
  assert.is(settings.get('tempTBool'),
            undefined,
            'tempTBool remove by obj');

  mockSettings.tempTBool = settings.add(mockSettings.tempTBoolSpec);
  assert.is(settings.get('tempTBool'),
            mockSettings.tempTBool,
            'tempTBool re-added 1');

  settings.remove('tempTBool');
  assert.is(settings.get('tempTBool'),
            undefined,
...
exports.testChange = function(options) {
  if (options.isFirefox || options.isRemote) {
