...
exports.testNodes = function(options) {
  return helpers.audit(options, [
    ...
    {
      setup:    'tse :root --nodes2 body',
      check: {
        input:  'tse :root --nodes2 body',
        hints:                         ' [options]',
        ...
        args: {
          ...
          node: { arg: ' :root', status: 'VALID' },
          nodes: { status: 'VALID' },
          nodes2: { arg: ' --nodes2 body', status: 'VALID' }
        }
      },
      exec: {
      },
      post: function(output) {
        if (!options.isRemote) {
          ...
          assert.is(output.args.nodes2.item(0).tagName, 'BODY', 'body tagName');
        }

        assert.is(output.data.args.node, ':root', 'node data');
        assert.is(output.data.args.nodes, 'Error', 'nodes data');
        assert.is(output.data.args.nodes2, 'body', 'nodes2 data');
      }
    },
    {
