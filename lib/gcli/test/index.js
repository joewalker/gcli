 ...
 */
exports.run = function(terminal, gcli, mocksAdded) {
  ...
  if (!mocksAdded) {
    options.gcli.addItems(mockCommands.items);
  }
  ...
  examiner.run(options).then(function() {
    ...
    // Leave the commands registered to help diagnose problems
    // options.gcli.removeItems(mockCommands.items);
  });
};
