...
const examiner = require('../testharness/examiner');
const KeyEvent = require('../util/util').KeyEvent;

const test = require('../commands/test');
const helpers = require('./helpers');
const mockCommands = require('./mockCommands');
const createTerminalAutomator = require('./automators/terminal').createTerminalAutomator;
const suite = require('./suite');

/**
 * Some tricks to make studying the command line state easier
 */
const addDebugAids = exports.addDebugAids = function(options) {
  window.createDebugCheck = () => {
    helpers._createDebugCheck(options).then(() => {
  ...
  window.summaryJson = function() {
    const args = [ 'Requisition: ' ];
    const summary = options.terminal.language.requisition._summaryJson;
...
exports.run = function(terminal, isRemote) {
  ...
  const closeIfPhantomJs = function() {
  ...
  setMocks(options, true).then(() => {
    return examiner.run(options).then(() => {
      const name = options.isPhantomjs ? '\nPhantomJS' : 'Browser';

      console.log(examiner.detailedResultLog(name));
      console.log(helpers.timingSummary);
      document.testStatus = examiner.status.name;

      setMocks(options, false).then(closeIfPhantomJs).catch(ex => {
        console.error(ex);
        closeIfPhantomJs();
      });
    });
...
function setMocks(options, state) {
  const command = 'mocks ' + (state ? 'on' : 'off');
  return options.requisition.updateExec(command).then(data => {
    // We're calling "mocks on" on the server, but we still need to
    // register the mockCommand converters on the client
    const requiredConverters = mockCommands.items.filter(item => {
...
function shutdownServer(requisition) {
  return requisition.updateExec('connect remote').then(output => {
    console.log('exec: connect remote -> ' + output.data);
    return requisition.updateExec('remote server stop').then(output2 => {
