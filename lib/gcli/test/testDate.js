...
exports.testParse = function(options) {
  const date = options.requisition.system.types.createType('date');
  return date.parseString('now').then(function(conversion) {
    ...
    // 1 min of each other
    const gap = new Date().getTime() - conversion.value.getTime();
...
exports.testMaxMin = function(options) {
  ...
  const date = types.createType({ name: 'date', max: max, min: min });
  assert.is(date.getMax(), max, 'max setup');

  const incremented = date.nudge(min, 1);
...
exports.testIncrement = function(options) {
  const date = options.requisition.system.types.createType('date');
  return date.parseString('now').then(function(conversion) {
    const plusOne = date.nudge(conversion.value, 1);
    const minusOne = date.nudge(plusOne, -1);

    // See comments in testParse
    const gap = new Date().getTime() - minusOne.getTime();
    assert.ok(gap < 60000, 'now is less than a minute away');
  });
};
