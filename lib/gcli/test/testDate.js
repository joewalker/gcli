...
// During a test run we may want to compare 2 dates, which should be very close
// but could well be a few milliseconds apart. How much fuzz should we allow?
// Mochitest will time out after 30 secs, so that seems like a decent upper
// limit, although 30 ms should probably do it. I don't think reducing the
/// limit from 30 secs will find any extra bugs
var SLOW_TEST_FUZZ = 30 * 1000;

// Tests that involve 'yesterday' and 'tomorrow' may also discover that a
// daylight savings change has made the same time tomorrow be 25 or 23 hours
// away, so we have added fuzz for those occasions
var DST_FUZZ = 60 * 60 * 1000;
    ...
    },
    {
      ...
      check: {
        ...
        args: {
          ...
          d1: {
            value: function(d1) {
              assert.ok(d1.getTime() - new Date().getTime() < SLOW_TEST_FUZZ,
                        'd1 time');
            },
            arg: ' now',
            status: 'VALID',
          ...
          d2: {
            value: function(d2) {
              assert.ok(d2.getTime() - new Date().getTime() < SLOW_TEST_FUZZ,
                        'd2 time');
            },
            arg: ' today',
            status: 'VALID',
    ...
    },
    {
      ...
      check: {
        ...
        args: {
          ...
          d1: {
            value: function(d1) {
              var compare = new Date().getTime() - ONE_DAY_IN_MILLIS;
              assert.ok(d1.getTime() - compare < (SLOW_TEST_FUZZ + DST_FUZZ),
                        'd1 time');
            },
            arg: ' yesterday',
            status: 'VALID',
          ...
          d2: {
            value: function(d2) {
              var compare = new Date().getTime() + ONE_DAY_IN_MILLIS;
              assert.ok(d2.getTime() - compare < (SLOW_TEST_FUZZ + DST_FUZZ),
                        'd2 time');
            },
            arg: ' tomorrow',
            status: 'VALID',
