...
function createExec(name) {
  return function(args, context) {
    ...
    Object.keys(args).map(argName => {
      ...
      const type = command.getParameterByName(argName).type;
      const promise = Promise.resolve(type.stringify(value, context));
    ...
    return Promise.all(promises).then(data => {
      const argValues = {};
      data.forEach(entry => argValues[entry.name] = entry.value);
...
mockCommands.items = [
  ...
  },
  {
    ...
    to: 'string',
    exec: function(testCommandOutput, context) {
      const argsOut = Object.keys(testCommandOutput.args)
                          .map(key => key + '=' + testCommandOutput.args[key])
                          .join(' ');
  ...
  },
  {
    ...
    delegateType: function(executionContext) {
      if (executionContext != null) {
        const option = executionContext.getArgsObject().optionType;
        if (option != null) {
          return option;
        }
      }
    ...
    exec: function(args, context) {
      if (args.method === 'reject') {
        return new Promise((resolve, reject) => {
          setTimeout(() => reject('rejected promise'), 10);
        });
      }

      if (args.method === 'rejecttyped') {
        return new Promise((resolve, reject) => {
          setTimeout(() => reject(context.typedData('number', 54)), 10);
        });
      }

      if (args.method === 'throwinpromise') {
        return new Promise((resolve, reject) => {
          setTimeout(() => resolve('should be lost'), 10);
        }).then(() => {
          const t = null;
          return t.foo;
        });
