 ...
 * Convert a view object to a string
 */
const stringViewStringConverter = {
  item: 'converter',
  from: 'stringView',
  to: 'string',
  exec: function(view, conversionContext) {
 ...
 */
const errorDomConverter = {
  item: 'converter',
  from: 'error',
  to: 'dom',
  exec: function(ex, conversionContext) {
    const node = util.createElement(conversionContext.document, 'p');
...
function getChainConverter(first, second) {
  ...
  return {
    ...
    to: second.to,
    exec: function(data, conversionContext) {
      const intermediate = first.exec(data, conversionContext);
...
Converters.prototype.add = function(converter) {
  let fromMatch = this._registered.from[converter.from];
  if (fromMatch == null) {
    fromMatch = {};
...
Converters.prototype.remove = function(converter) {
  const fromMatch = this._registered.from[converter.from];
  if (fromMatch == null) {
    return;
  }
...
Converters.prototype.get = function(from, to) {
  const fromMatch = this._registered.from[from];
  if (fromMatch == null) {
    return this._getFallbackConverter(from, to);
  }

  let converter = fromMatch[to];
  if (converter == null) {
...
Converters.prototype.getAll = function() {
  return Object.keys(this._registered.from).map(name => this._registered.from[name]);
};
...
Converters.prototype.convert = function(data, from, to, conversionContext) {
  try {
    ...
    const converter = this.get(from, to);
    return host.exec(() => converter.exec(data, conversionContext));
  }
  catch (ex) {
    const converter = this.get('error', to);
    return host.exec(() => converter.exec(ex, conversionContext));
  }
};
