...
var strings = require('i18n!gcli/nls/strings');
...
function replace(str, swaps) {
  return str.replace(/\{[^}]*\}/g, function(name) {
    ...
    if (replacement == null) {
      console.log('Missing lookup \'' + name +
              '\' given ' + JSON.stringify(swaps));
    }
    return replacement;
  });
}
 ...
 */
exports.lookup = function(key, swaps) {
  var str = lookupKey(key);
 ...
 * the English and the French words have the same etimology)
 */
exports.lookupPlural = function(key, ord, swaps) {
  var index = getPluralRule().get(ord);
  var words = lookupKey(key);
  var str = words[index];
