...
Settings.prototype.getAll = function(filter) {
  ...
  all.sort((s1, s2) => s1.name.localeCompare(s2.name));
  return all;
};
...
Settings.prototype.add = function(prefSpec) {
  const type = this._types.createType(prefSpec.type);
  const setting = new Setting(this, prefSpec.name, type, prefSpec.description,
...
LocalSettingStorage.prototype.load = function(values) {
  ...
  const gcliSettings = localStorage.getItem('gcli-settings');
  if (gcliSettings != null) {
    const parsed = JSON.parse(gcliSettings);
    Object.keys(parsed).forEach(name => values[name] = parsed[name]);
  }
};
