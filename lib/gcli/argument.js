 ...
 * @param options Values to use in creating a new Argument. Properties:
 * - text: The new text value (required)
 * - prefixSpace: Should the prefix be altered to have a space?
 */
Argument.prototype.beget = function(options) {
  ...
  var quote = (options.text.indexOf(' ') >= 0 || options.text.length == 0) ?
      '\'' : '';
  ...
  return new Argument(options.text, prefix, suffix);
};
...
ScriptArgument.prototype.beget = function(options) {
  ...
  return new ScriptArgument(options.text, prefix, suffix);
};
