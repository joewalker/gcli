...
Fields.prototype.remove = function(field) {
  if (typeof field !== 'string') {
    this._fieldCtors = this._fieldCtors.filter(test => test !== field);
  }
...
Fields.prototype.get = function(type, options) {
  ...
  let highestClaim = -1;
  this._fieldCtors.forEach(fieldCtor => {
    const context = (options.requisition == null) ?
                  null : options.requisition.executionContext;
    const claim = fieldCtor.claim(type, context);
