...
const baseLanguage = {
  ...
  handleTab: function() {
    return this.terminal.unsetChoice().then(() => RESOLVED)
                                      .catch(util.errorHandler);
  },

  handleInput: function(input) {
    if (input === ':') {
      return this.terminal.setInput('').then(() => this.terminal.pushLanguage('commands'));
    }
...
Languages.prototype.getAll = function() {
  return Object.keys(this._registered).map(name => this._registered[name]);
};
...
Languages.prototype.createLanguage = function(name, terminal) {
  ...
  const language = (typeof name === 'string') ? this._registered[name] : name;
  if (!language) {
  ...
  if (typeof newInstance.constructor === 'function') {
    const reply = newInstance.constructor(terminal);
    return Promise.resolve(reply).then(() => newInstance);
  }
  else {
