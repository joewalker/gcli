...
const prism = require('../util/prism').Prism;
...
exports.items = [
  {
    ...
    exec: function(input) {
      return this.evaluate(input).then(response => {
        let output = (response.exception != null) ?
                      response.exception.class :
                      response.output;

        const isSameString = typeof output === 'string' &&
                           input.substr(1, input.length - 2) === output;
        const isSameOther = typeof output !== 'string' &&
                          input === '' + output;
        ...
        const grammar = prism.languages[this.name];
