...
const Requisition = require('../cli').Requisition;
const CommandAssignment = require('../cli').CommandAssignment;
 ...
 */
let commandHtmlPromise;

const commandLanguage = exports.commandLanguage = {
  ...
  constructor: function(terminal) {
    ...
    return commandHtmlPromise.then(commandHtml => {
      ...
      const mapping = cli.getMapping(this.requisition.executionContext);
  ...
  textChanged: function() {
    ...
    const newStr = this.requisition.toString();
    const input = this.terminal.getInputState();
  ...
  caretMoved: function(start) {
    ...
    const newAssignment = this.requisition.getAssignmentAt(start);
    ...
    if (this.assignment !== newAssignment) {
      ...
      const isNew = (this.assignment !== newAssignment);
    ...
    const error = (this.assignment.status === Status.ERROR);
  ...
  updateHints: function() {
    ...
    let field = this.terminal.field;
    if (field) {
      field.onFieldChange.remove(this.terminal.fieldChanged, this.terminal);
      field.destroy();
    }

    const fields = this.terminal.system.fields;
    ...
    const contents = this.terminal.tooltipTemplate.cloneNode(true);
  ...
  handleTab: function() {
    ...
    this.terminal._caretChange = Caret.TO_ARG_END;
    const inputState = this.terminal.getInputState();
    ...
    // of the event loop
    const index = this.terminal.getChoiceIndex();
  ...
  getCompleterTemplateData: function() {
    const input = this.terminal.getInputState();
    const start = input.cursor.start;
    const index = this.terminal.getChoiceIndex();

    return this.requisition.getStateData(start, index).then(data => {
      ...
      data.statusMarkup.forEach(member => {
  ...
  fieldChanged: function(ev) {
    const opts = { matchPadding: true };
    this.requisition.setAssignment(this.assignment, ev.conversion.arg, opts).then(() => {
      this.textChanged();
    });

    const isError = ev.conversion.message != null && ev.conversion.message !== '';
  ...
  outputted: function(ev) {
    ...
    const template = this.commandDom.cloneNode(true);
    const templateOptions = { stack: 'terminal.html#outputView' };

    const context = this.requisition.conversionContext;
    const data = {
    ...
    ev.output.promise.then(() => {
      const document = data.rowoutEle.ownerDocument;
