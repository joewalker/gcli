...
NumberType.prototype.parse = function(arg) {
  ...
  if (isNaN(value)) {
    return new Conversion(this.min, arg, Status.ERROR,
        l10n.lookup('types_number_nan', { text: arg.text }));
  }

  if (this.max != null && value > this.max) {
    return new Conversion(value, arg, Status.ERROR,
        l10n.lookup('types_number_max', { value: value, max: this.max }));
  }

  if (this.min != null && value < this.min) {
    return new Conversion(value, arg, Status.ERROR,
        l10n.lookup('types_number_min', { value: value, max: this.max }));
  }
 ...
 * single function.
 * @return A map of names to values. It is NOT safe to mutate this object
 */
...
/**
 * Like getLookup() except that rather than returning a map of name->value,
...
SelectionType.prototype.parse = function(arg) {
  ...
  return new Conversion(null, arg, Status.ERROR,
      l10n.lookup('types_selection_nomatch', { text: arg.text }));
};
