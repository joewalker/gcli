 ...
 * <ul>
 * <li>onCommandChange: The command has changed. It is likely that a UI
 ...
 * changes.
 * <li>onInputChange: The text to be mirrored in a command line has changed.
 * </ul>
 *
...
function Requisition(environment, doc) {
  ...
  this.onCommandChange = util.createEvent('Requisition.onCommandChange');
  this.onInputChange = util.createEvent('Requisition.onInputChange');
}
...
Requisition.prototype._onAssignmentChange = function(ev) {
  ...
  // Both for argument position and the onInputChange event, we only care
 ...
 */
Requisition.prototype.complete = function(cursor) {
  ...
  if (predictions.length > 0) {
    ...
    var arg = assignment.getArg().beget(predictions[0].name);
    var conversion = new Conversion(predictions[0].value, arg);
