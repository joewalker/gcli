...
Requisition.prototype._onAssignmentChange = function(ev) {
    ...
    function removeShortName(argIdx) {
...
Requisition.prototype._assign = function(args) {
        ...
        while (i < args.length) {
            if (assignment.param.isKnownAs(args[i].text)) {
                ...
                if (assignment.param.type instanceof BooleanType) {
                    ...
                    // this means, the current arg can't be used.
                    // ??? Is quiting here okay?
                    if (i >= args.length) {
                        return;
                    }
                    // Disable this for now as it doesn't always picks the
                    // next arg as it should.
                    // if (i + 1 >= args.length) {
    ...
    Object.keys(handledArgs).forEach(function(idx) {
        args.splice(parseInt(idx), 1);
    });
