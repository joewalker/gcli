...
CommandType.prototype.parse = function(arg) {
    ...
    if (matches === 1) {
        // But is it an exact match, or just the only one?
        var match = completions[arg.text];
        if (match) {
            var status = match.exec ? Status.VALID : Status.INCOMPLETE;
            return new Conversion(match, arg, status, '', completer);
        }
    }
