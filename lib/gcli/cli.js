...
Requisition.prototype.setAssignment = function(assignment, arg, options) {
  ...
  if (arg == null) {
    ...
    return Q.resolve(assignment.param.type.parse(arg)).then(function(conversion) {
...
Requisition.prototype.complete = function(cursor, predictionChoice) {
  ...
  return Q.resolve(predictionPromise).then(function(prediction) {
    ...
    if (prediction == null) {
      ...
      if (assignment.isInName()) {
        ...
        promise = this.setAssignment(assignment, newArg, { argUpdate: true });
      }
    }
    else {
      ...
      promise = this.setAssignment(assignment, arg, { argUpdate: true });
      ...
      if (!prediction.incomplete) {
        promise = promise.then(function() {
          ...
          // TODO: decide what to do with this ignored promise
...
Requisition.prototype._addSpace = function(assignment) {
  ...
  if (arg !== assignment.conversion.arg) {
    var promise = this.setAssignment(assignment, arg, { argUpdate: true });
    util.synchronize(promise);
  }
};
...
Requisition.prototype.exec = function(input) {
  ...
  // TODO: find some way to pass the exec promise back to the outside
  // Both update functions below have their promises ignored
  if (input) {
  ...
  // TODO: Don't ignore this promise
  this.update('');
  return output;
};
...
Requisition.prototype._split = function(args) {
  ...
  if (args[0].type === 'ScriptArgument') {
    ...
    return this.setAssignment(this.commandAssignment, conversion);
  }
...
Requisition.prototype._assign = function(args) {
  ...
  if (this.assignmentCount === 1) {
    ...
    if (assignment.param.type instanceof StringType) {
      ...
      outstanding.push(this.setAssignment(assignment, arg));
      return util.all(outstanding);
    }
  }
  ...
  unassignedParams.forEach(function(name) {
    ...
    if (!assignment.param.isPositionalAllowed) {
      outstanding.push(this.setAssignment(assignment, null));
      return;
    }
  ...
  Object.keys(arrayArgs).forEach(function(name) {
    var assignment = this.getAssignment(name);
    outstanding.push(this.setAssignment(assignment, arrayArgs[name]));
  }, this);
