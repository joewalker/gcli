 ...
 *   be sent
 * - commands An instance of Commands that specifies the commands that are allowed in
 *   this Requisition
 * @constructor
 */
function Requisition(options) {
  ...
  this.commands = options.commands || centralCommands;
...
Requisition.prototype._split = function(args) {
  ...
  if (args[0].type === 'ScriptArgument') {
    ...
    conversion = new Conversion(getEvalCommand(this.commands), new ScriptArgument());
...
Requisition.prototype.exec = function(options) {
  ...
  if (options) {
    ...
    if (options.command != null) {
      ...
      command = this.commands.get(options.command);
      if (!command) {
