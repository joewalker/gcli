...
const Parameter = require('./commands/commands').Parameter;
const CommandOutputManager = require('./commands/commands').CommandOutputManager;

const Status = require('./types/types').Status;
const Conversion = require('./types/types').Conversion;
const commandModule = require('./types/command');
const selectionModule = require('./types/selection');

const Argument = require('./types/types').Argument;
const ArrayArgument = require('./types/types').ArrayArgument;
const NamedArgument = require('./types/types').NamedArgument;
const TrueNamedArgument = require('./types/types').TrueNamedArgument;
const MergedArgument = require('./types/types').MergedArgument;
const ScriptArgument = require('./types/types').ScriptArgument;

const RESOLVED = Promise.resolve(undefined);

/**
 * Helper to produce a `deferred` object using DOM Promise
 */
function defer() {
  let resolve, reject;
...
exports.getMapping = function(context) {
  const index = instanceIndex(context);
  if (index === -1) {
 ...
 */
const addMapping = function(requisition) {
 ...
 */
const removeMapping = function(requisition) {
  const index = instanceIndex(requisition.conversionContext);
...
const evalCmd = {
  ...
  exec: function(args, context) {
    const reply = customEval(args.javascript);
...
function UnassignedAssignment(requisition, arg) {
  const isIncompleteName = (arg.text.charAt(0) === '-');
  ...
  // case)
  if (isIncompleteName) {
    const lookup = commandModule.getDisplayedParamLookup(requisition);
    const predictions = selectionModule.findPredictions(arg, lookup);
...
Requisition.prototype._beginChange = function() {
  const updateId = this._nextUpdateId;
  this._nextUpdateId++;
  return updateId;
};
...
Requisition.prototype.getAssignment = function(nameOrNumber) {
  const name = (typeof nameOrNumber === 'string') ?
    nameOrNumber :
...
Object.defineProperty(Requisition.prototype, 'status', {
  get: function() {
    ...
    if (this._unassigned.length !== 0) {
      let isAllIncomplete = true;
      this._unassigned.forEach(assignment => {
    ...
    this.getAssignments(true).forEach(assignment => {
      const assignStatus = assignment.getStatus();
...
Requisition.prototype._getFirstBlankPositionalAssignment = function() {
  let reply = null;
  Object.keys(this._assignments).some(name => {
    const assignment = this.getAssignment(name);
...
Requisition.prototype.getAssignmentAt = function(cursor) {
  ...
  for (i = 0; i < this._args.length; i++) {
    ...
    if (arg.assignment.arg.type === 'NamedArgument') {
      ...
      const nextAssignment = this._getFirstBlankPositionalAssignment();
...
Requisition.prototype.toCanonicalString = function() {
  const cmd = this.commandAssignment.value ?
  ...
  let lineSuffix = '';
  if (cmd === '{') {
    const scriptSuffix = this.getAssignment(0).arg.suffix;
  ...
  const argPromise = util.promiseEach(this.getAssignments(), assignment => {
    ...
    const val = assignment.param.type.stringify(assignment.value, ctx);
    return Promise.resolve(val).then(str => ' ' + str);
  });

  return argPromise.then(strings => cmd + strings.join('') + lineSuffix);
};
...
Object.defineProperty(Requisition.prototype, '_summaryJson', {
  get: function() {
    const summary = {
      $args: this._args.map(arg => arg._summaryJson),
      _command: this.commandAssignment._summaryJson,
      _unassigned: this._unassigned.forEach(assignment => assignment._summaryJson)
    };
...
Requisition.prototype._setAssignmentInternal = function(assignment, conversion) {
  const oldConversion = assignment.conversion;
  ...
  if (assignment === this.commandAssignment) {
    ...
    const command = this.commandAssignment.value;
    if (command) {
      for (let i = 0; i < command.params.length; i++) {
        const param = command.params[i];
        const newAssignment = new Assignment(param);
...
Requisition.prototype.setAssignment = function(assignment, arg, options) {
  options = options || {};
  if (!options.internal) {
    const originalArgs = assignment.arg.getArgs();

    // Update the args array
    const replacementArgs = arg.getArgs();
    const maxLen = Math.max(originalArgs.length, replacementArgs.length);
    for (let i = 0; i < maxLen; i++) {
      ...
      const index = this._args.indexOf(originalArgs[i]);
      if (index === -1) {
  ...
  const updateId = options.internal ? null : this._beginChange();

  const setAssignmentInternal = conversion => {
  ...
  if (arg == null) {
    const blank = assignment.param.type.getBlank(this.executionContext);
  ...
  const parsed = assignment.param.type.parse(arg, this.executionContext);
...
Requisition.prototype.typedEndsWithSeparator = function() {
  ...
  const lastArg = this._args.slice(-1)[0];
...
Requisition.prototype.getInputStatusMarkup = function(cursor) {
  const argTraces = this.createInputArgTrace();
  ...
  const cTrace = argTraces[cursor];

  const markup = [];
  for (let i = 0; i < argTraces.length; i++) {
    const argTrace = argTraces[i];
    const arg = argTrace.arg;
    let status = Status.VALID;
    ...
    if (argTrace.part === 'text' && arg.assignment != null) {
      ...
      if (status === Status.INCOMPLETE) {
        ...
        const isNamed = (cTrace.arg.assignment.arg.type === 'NamedArgument');
        const isInside = cTrace.part === 'text' ||
...
Requisition.prototype.getStateData = function(start, rank) {
  const typed = this.toString();
  const current = this.getAssignmentAt(start);
  const context = this.executionContext;
  const predictionPromise = (typed.trim().length !== 0) ?
  ...
  return predictionPromise.then(prediction => {
    ...
    if (typed.trim().length !== 0) {
      const cArg = current.arg;

      if (prediction) {
        let tabText = prediction.name;
        const existing = cArg.text;
        ...
        if (existing !== tabText) {
          ...
          const inputValue = existing.replace(/^\s*/, '');
          const isStrictCompletion = tabText.indexOf(inputValue) === 0;
          if (isStrictCompletion && start === typed.length) {
            // Display the suffix of the prediction as the completion
            const numLeadingSpaces = existing.match(/^(\s*)/)[0].length;
    ...
    this.getAssignments().forEach(assignment => {
      ...
      const text = (assignment.param.isDataRequired) ?
    ...
    if (command && command.hasNamedParameters) {
      command.params.forEach(param => {
        const arg = this.getAssignment(param.name).arg;
    ...
    const unclosedJs = command && command.name === '{' &&
...
Requisition.prototype._addSpace = function(assignment) {
  const arg = assignment.arg.beget({ suffixSpace: true });
...
Requisition.prototype.complete = function(cursor, rank) {
  const assignment = this.getAssignmentAt(cursor.start);

  const context = this.executionContext;
  const predictionPromise = assignment.getPredictionRanked(context, rank);
  return predictionPromise.then(prediction => {
    const outstanding = [];
    ...
    return Promise.all(outstanding).then(() => true);
  });
};
...
Requisition.prototype.nudge = function(assignment, by) {
  const ctx = this.executionContext;
  const val = assignment.param.type.nudge(assignment.value, by, ctx);
  return Promise.resolve(val).then(replacement => {
    if (replacement != null) {
      const value = assignment.param.type.stringify(replacement, ctx);
      return Promise.resolve(value).then(str => {
        const arg = assignment.arg.beget({ text: str });
        return this.setAssignment(assignment, arg);
      });
    }
  });
...
Requisition.prototype.update = function(typed) {
  ...
  const updateId = this._beginChange();

  this._args = exports.tokenize(typed);
  const args = this._args.slice(0); // i.e. clone

  this._split(args);

  return this._assign(args).then(() => this._endChangeCheckOrder(updateId));
};
...
Requisition.prototype.clear = function() {
  ...
  const conversion = commandModule.parse(this.executionContext, arg, false);
...
exports.tokenize = function(typed) {
  ...
  let start = 0;      // Where did this section start?
  let prefix = '';    // Stuff that comes before the current argument
  const args = [];    // The array that we're creating
  let blockDepth = 0; // For JS with nested {}
    ...
    if (i >= typed.length) {
      ...
      if (mode === In.WHITESPACE) {
        if (i !== start) {
          ...
          const extra = typed.substring(start, i);
...
Requisition.prototype._split = function(args) {
  ...
  if (args[0].type === 'ScriptArgument') {
    ...
    const command = this.system.commands.get(evalCmd.name);
  ...
  while (argsUsed <= args.length) {
    const arg = (argsUsed === 1) ?
              args[0] :
              new MergedArgument(args, 0, argsUsed);

    if (this.prefix != null && this.prefix !== '') {
      const prefixArg = new Argument(this.prefix, '', ' ');
      const prefixedArg = new MergedArgument([ prefixArg, arg ]);
...
Requisition.prototype._assign = function(args) {
  ...
  if (this.assignmentCount === 1) {
    const assignment = this.getAssignment(0);
    if (assignment.param.type.name === 'string') {
      const arg = (args.length === 1) ? args[0] : new MergedArgument(args);
  ...
  let unassignedParams = this.getParameterNames();

  // We collect the arguments used in arrays here before assigning
  const arrayArgs = {};

  // Extract all the named parameters
  const assignments = this.getAssignments(false);
  const namedDone = util.promiseEach(assignments, assignment => {
    ...
    while (i < args.length) {
      ...
      let arg = args.splice(i, 1)[0];
      unassignedParams = unassignedParams.filter(test => {
      ...
      if (assignment.param.type.name === 'array') {
        let arrayArg = arrayArgs[assignment.param.name];
        if (!arrayArg) {
  ...
  const positionalDone = namedDone.then(() => {
    return util.promiseEach(unassignedParams, name => {
      const assignment = this.getAssignment(name);
      ...
      const isIncompleteName = assignment.param.type.name === 'number' ?
  ...
  const arrayDone = positionalDone.then(() => {
    return util.promiseEach(Object.keys(arrayArgs), name => {
      const assignment = this.getAssignment(name);
  ...
  return arrayDone.then(() => this._addUnassignedArgs(args));
};
...
Requisition.prototype.exec = function(options) {
  ...
  if (this.status !== Status.VALID) {
    ...
    try {
      return host.exec(() => command.exec(args, this.executionContext))
                 .then(onDone, onError);
    }
    catch (ex) {
      const data = (typeof ex.message === 'string' && ex.stack != null) ?
                 ex.message : ex;
...
Requisition.prototype._contextUpdateExec = function(typed, options) {
  ...
  const child = new Requisition(this.system, reqOpts);
  return child.updateExec(typed, options).then(reply => {
    child.destroy();
    return reply;
  });
};
...
Requisition.prototype.updateExec = function(input, options) {
  return this.update(input).then(() => this.exec(options));
};
...
Output.prototype.convert = function(type, conversionContext) {
  const converters = conversionContext.system.converters;
