...
var commands = require('gcli/commands');
var evalCommand;
...
exports.startup = function() {
  ...
  evalCommand = commands.getEvalCommand();
};
...
CommandType.prototype._getLookupArray = SelectionType.prototype._getLookupArray;
...
/**
 * Create a map of command-name to command that we can mutate if desired
 */
CommandType.prototype._getLookup = function() {
  // This is ripe for optimization, canon.getCommands() creates an array from
  // an object and we just turn it back again.
  var commands = {};
  var cmdarr = canon.getCommands();
  cmdarr.sort(function(command1, command2) {
    return command1.name.localeCompare(command2.name);
  });
  cmdarr.forEach(function(command) {
...
/**
 * Trim a list of commands (as from _getLookup) according to those that
 * match the provided arg.
 */
...
CommandType.prototype.parse = function(arg) {
  ...
  var matchCount = Object.keys(predictions).length;
  if (matchCount === 0) {
  ...
  if (matchCount === 1) {
    ...
    if (predictions[0].name === arg.text) {
      ...
      if (typeof value.exec === 'function') {
        status = Status.VALID;
      }
    }
  }

  // Exact match when there are several options is incomplete
  if (predictions[0].name === arg.text) {
    value = predictions[0].value;
    status = Status.VALID;
  }

  return new Conversion(value, arg, status, msg, predictFunc);
};
...
Requisition.prototype._split = function(args) {
  ...
  if (args[0] instanceof ScriptArgument) {
    ...
    this.commandAssignment.setConversion(new Conversion(evalCommand, args[0]));
    return;
  }
