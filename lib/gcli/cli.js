...
Requisition.prototype.complete = function(cursor, predictionChoice) {
  ...
  if (predictions.length > 0) {
    ...
    // completed, because we could be completing a named argument, so we need
    // to look for the first blank positional parameter, but if there isn't one
    // of those then we just add to the suffix of the current
