...
var CommandType = require('./types/command');

var Argument = require('./argument').Argument;
var ArrayArgument = require('./argument').ArrayArgument;
var NamedArgument = require('./argument').NamedArgument;
var TrueNamedArgument = require('./argument').TrueNamedArgument;
var MergedArgument = require('./argument').MergedArgument;
var ScriptArgument = require('./argument').ScriptArgument;
...
exports.getMapping = function(context) {
  ...
  return index === -1 ? undefined : instances[index].mapping;
};

/**
 * Add a requisition context->terminal mapping
 */
exports.addMapping = function(requisition, mapping) {
  instances.push({
 ...
 */
exports.removeMapping = function(requisition) {
...
Requisition.prototype.decrement = function(assignment) {
  var val = assignment.param.type.decrement(assignment.value,
...
Requisition.prototype.increment = function(assignment) {
  var val = assignment.param.type.increment(assignment.value,
                                            this.executionContext);
  return promise.resolve(val).then(function(replacement) {
    if (replacement != null) {
      var str = assignment.param.type.stringify(replacement,
...
Requisition.prototype.clear = function() {
  ...
  var conversion = CommandType.parse(arg, false);
...
Requisition.prototype._split = function(args) {
  ...
  while (argsUsed <= args.length) {
    ...
    if (this.prefix != null && this.prefix !== '') {
      ...
      conversion = CommandType.parse(prefixedArg, false);
      if (conversion.value == null) {
        conversion = CommandType.parse(arg, false);
      }
    }
    else {
      conversion = CommandType.parse(arg, false);
