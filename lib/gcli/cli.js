...
Requisition.prototype._onAssignmentChange = function(ev) {
    ...
    // Remember where we found the short name like -a
    var indexShort = null;
    var isBool = ev.assignment.param.type instanceof BooleanType;
    var param = ev.assignment.param;
    for (var i = 0; i < this._args.length; i++) {
        var match;
        var text = this._args[i].text;
        // BooleanTypes need some special handling here, as they might be more
        // then only one parameter bound to one argument.
        if (isBool) {
            match = param.isKnownAs(this._args[i].text);
        } else {
            if (param.isKnownAs(this._args[i].text)) {
                if (/^-\w+/.test(text)) {
                    indexShort = i;
                    continue;
                } else {
                ...
                match = this._args[i].assignment === ev.assignment
            }
        }
        if (match) {
            if (i < index) {
            ...
            // Check if we remove a boolean parameter. This might be set
            // using a short form like "-am". If the param "a" should get removed
            // we have to remove only the "a" but keep the rest.
            if (isBool && /^-\w+/.test(text)) {
                if (text.length != 2) {
                    var text = this._args[i].text;
                    var short = ev.assignment.param.short;
                    this._args[i].text = text.replace(short, "");
                    break;
                }
            }
    ...
    if (!param.isPositionalAllowed() && !isBool && indexShort == null) {
        newArgs[0].prefix = " --" + param.name + newArgs[0].prefix;
    }
...
Requisition.prototype._assign = function(args) {
        ...
        while (i < args.length) {
            if (assignment.param.isKnownAs(args[i].text)) {
                ...
                if (assignment.param.type instanceof BooleanType) {
                    ...
                    // Skip args that are already handled.
                    // This is needed in case of "-ms hello foo" where in the
                    // first pass the 'hello' is assigned to '-m' and later on
                    // the '-s' should get assigned to 'foo'.
                    while (handledArgs[i]) {
                        i++;
                    }

                    // In case there should be an argument, but there isn't,
                    // create a new/empty argument.
                    if (i >= args.length) {
                        valueArg = new Argument();
                    } else {
                        valueArg = args[i];
                        // Mark this argument as handled.
                        handledArgs[i] =  true;
                    }
    ...
    // Remove args that were handled.
    var argsLeft = [];
    for (var i = 0; i < args.length; i++) {
        if (!(i in handledArgs)) {
            argsLeft.push(args[i]);
        }
    }
    args = argsLeft;
