...
var remoteHandles = {};
var nextRemoteHandle = 1;

/**
 * Get a remote handle for a function. Example usage:
 *
 *   var example = {
 *     greet: function(name1, name2) { return 'Hello, ' + name1 + ' ' + name2; }
 *   };
 *
 *   var handle = getHandle(example.greet, greet, 'Fred');
 *
 *   // later
 *
 *   callRemoteHandle(handle, 'Blogs');  // 'Hello, Fred Blogs'
 *
 * This can also be used remotely
 */
exports.getHandle = function(func, scope, args) {
  var id = '#' + (nextRemoteHandle++);
  remoteHandles[id] = {
    func: func,
    scope: scope,
...
exports.removeHandle = function(id) {
  delete remoteHandles[id];
};

exports.callHandle = function(context, id, args) {
  args = args || [];
  args = args.map(function(value) {
    return (value === contextProxy) ? context : value;
  });

  var remoteHandle = remoteHandles[id];
  if (remoteHandle == null) {
    throw new Error('Unknown remote handle ' + id);
  }
  args = remoteHandle.args.concat(args);
  var reply = remoteHandle.func.apply(remoteHandle.scope, args);
  return promise.resolve(reply);
};

exports.callRemoteHandle = function(connection, id, args) {
  args = args || [];
  // 'context' is not remotable, (it's a link to a requisition) so we replace
  // with a magic string which we recognize on the other end
  args = args.map(function(value) {
    return (value !== null && typeof value.typedData === 'function') ?
           contextProxy : value;
  });
  return connection.call('call', { id: id, args: args });
};

var contextProxy = '__CONTEXT__T4PL5TM__';
