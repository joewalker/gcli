...
function nameFunction(handler) {
  const scope = handler.scope ? handler.scope.constructor.name + '.' : '';
  const name = handler.func.name;
  if (name) {
    return scope + name;
  }
  for (const prop in handler.scope) {
...
exports.createEvent = function(name) {
  ...
  event.remove = function(func, scope) {
    ...
    let found = false;
    handlers = handlers.filter(test => {
      const match = (test.func === func && test.scope === scope);
      if (match) {
        found = true;
      }
      return !match;
  ...
  event.resumeFire = function() {
    ...
    if (heldEvents.length === 1) {
      ...
      const first = heldEvents[0];
      const last = heldEvents[heldEvents.length - 1];
      if (eventCombiner) {
...
exports.promiseEach = function(array, action, scope) {
  ...
  array.forEach((member, i) => {
    promise = promise.then(() => {
      const reply = action.call(scope, member, i, array);
      return Promise.resolve(reply).then(data => allReply[i] = data);
    });
  });

  return promise.then(() => allReply);
};
...
exports.importCss = function(cssText, doc, id) {
  ...
  const style = exports.createElement(doc, 'style');
  style.id = id;
...
exports.setTextContent = function(elem, text) {
  ...
  const child = elem.ownerDocument.createTextNode(text);
...
exports.setContents = function(elem, contents) {
  ...
  if ('innerHTML' in elem) {
    ...
    try {
      let ns = elem.ownerDocument.documentElement.namespaceURI;
      if (!ns) {
        ns = exports.NS_XHTML;
      }
      exports.clearElement(elem);
      ...
      const child = range.createContextualFragment(contents).firstChild;
