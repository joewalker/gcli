...
//   var error = PendingErrors.extract(aValue);
//   var {message, date, fileName, stack, lineNumber} = error;
...
//    console.log("Full stack: " + (stack||"not available") + "\n");
//    console.log("*************************\n");
//    return;
//  }
//  if (stack) {
...
var Promise = Object.freeze({
  ...
  all: function (aValues)
  {
    ...
    var countdown = aValues.length;
    var deferred = Promise.defer();
    var resolutionValues = new Array(countdown);

    function checkForCompletion(aIndex, aValue) {
    ...
    for (var i = 0; i < aValues.length; i++) {
      ...
      if (value && typeof(value.then) == "function") {
        var resolve = checkForCompletion.bind(null, index);
        value.then(resolve, deferred.reject);
      } else {
        // Given value is not a promise, forward it as a resolution value.
        checkForCompletion(index, value);
      }
    }
...
var PromiseWalker = {
  ...
  scheduleWalkerLoop: function()
  {
    ...
    setTimeout(function() {          // Services.tm.currentThread.dispatch(this.walkerLoop,
    this.walkerLoop(); }.bind(this), 0);        //                                    Ci.nsIThread.DISPATCH_NORMAL);
    //this.walkerLoop();
  ...
  then: function (aOnResolve, aOnReject)
  {
    ...
    if (this[N_STATUS] != STATUS_PENDING) {
      ...
      if (this[N_WITNESS] != null) {
        var id = this[N_WITNESS][0]; var witness = this[N_WITNESS][1];
        this[N_WITNESS] = null;
        witness.forget();
