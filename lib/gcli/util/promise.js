...
/* jshint quotmark:double, newcap:false */
"use strict";

/**
 * This file is taken from Promise-backend.js from mozilla-central, and
 * modified to fit the web.
 */
...
var Name = function(n) { return "{private:" + n + ":" + salt + "}"; };
var N_STATUS = Name("status");
var N_VALUE = Name("value");
var N_HANDLERS = Name("handlers");
var N_WITNESS = Name("witness");
...
var PendingErrors = {
  ...
  register: function(error) {
    var id = "pending-error-" + (this._counter++);
    //
    ...
    try { // Defend against non-enumerable values
      if (error && false /*error instanceof Ci.nsIException*/) {
        ...
        ["fileName", "stack", "lineNumber"].forEach(function(k) {
      ...
      if (!value.stack) {
        ...
        if (error && error.location &&
            false /*error.location instanceof Ci.nsIStackFrame*/) {
   ...
   */
  report: function(id) {
    var value = this._map.get(id);
    if (!value) {
      return; // The error has already been reported
    }
    delete this._map[id];
    this._observers.values().forEach(function(obs) {
      obs(value);
    }.bind(this));
  },
...
PendingErrors.addObserver(function(details) {
  ...
  if (true /*!error || !Services.console*/) {
    // Too late during shutdown to use the nsIConsole
    console.log("*************************\n");
    console.log("A promise chain failed to handle a rejection\n\n");
    console.log("On: " + details.date + "\n");
    console.log("Full message: " + details.message + "\n");
    console.log("See https://developer.mozilla.org/Mozilla/JavaScript_code_modules/Promise.jsm/Promise\n");
    console.log("Full stack: " + (details.stack||"not available") + "\n");
    console.log("*************************\n");
    return;
  }
 ...
 */
var Promise = function Promise(aExecutor)
{
  ...
  var resolve = PromiseWalker.completePromise
                             .bind(PromiseWalker, this, STATUS_RESOLVED);
  var reject = PromiseWalker.completePromise
...
{
  var handler = new Handler(this, aOnResolve, aOnReject);
...
Promise.all = function (aValues)
{
  ...
  return new Promise(function(resolve, reject) {
    var values = Array.isArray(aValues) ? aValues : spread(aValues);
    ...
    var makeCheck = function(index) {
      return function(val) { return checkForCompletion(val, index); };
    };

    for (var i = 0; i < values.length; i++) {
      var index = i;
      var value = values[i];
      var resolver = makeCheck(index);
...
var PromiseWalker = {
  ...
  completePromise: function (aPromise, aStatus, aValue)
  {
    ...
    if (aPromise[N_HANDLERS].length > 0) {
      ...
      console.error("Promise rejected with no handlers.", aPromise, aValue); // var id = PendingErrors.register(aValue);
      /*var id =*/ PendingErrors.register(aValue);
      // var witness =
  ...
  {
    this.stacks = stacks; // Make it easy to see in a debugger
    var stack;
  ...
  schedulePromise: function (aPromise)
  {
    ...
    aPromise[N_HANDLERS].forEach(function(handler) {
...
Handler.prototype = {
  ...
  process: function()
  {
    ...
    var nextStatus = this.thisPromise[N_STATUS];
    var nextValue = this.thisPromise[N_VALUE];
    ...
    try {
      ...
      if (ex && typeof ex == "object" && "name" in ex &&
        ...
        console.log("*************************\n");
        console.log("A coding exception was thrown in a Promise " +
             ((nextStatus == STATUS_RESOLVED) ? "resolution":"rejection") +
             " callback.\n\n");
        console.log("Full message: " + ex + "\n");
        console.log("See https://developer.mozilla.org/Mozilla/JavaScript_code_modules/Promise.jsm/Promise\n");
        console.log("Full stack: " + (("stack" in ex)?ex.stack:"not available") + "\n");
        console.log("*************************\n");
