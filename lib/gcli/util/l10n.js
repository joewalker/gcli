...
const additions = require('../nls/strings').root;
Object.keys(additions).forEach(key => strings[key] = additions[key]);
...
exports.getPreferredLocales = function() {
  ...
  const reply = parts.map((part, index) => {
...
function format(str, swaps) {
  ...
  let index = 0;
  str = str.replace(/%S/g, () => swaps[index++]);
  // Then %n$S style strings
  str = str.replace(/%([0-9])\$S/g, (match, idx) => swaps[idx - 1]);
  return str;
}
...
exports.lookupPlural = function(key, ord, swaps) {
  const index = getPluralRule().get(ord);
  const words = exports.lookup(key);
