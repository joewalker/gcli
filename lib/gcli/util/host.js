...
const isWebpack = (require.cache != null);

const ATTR_NAME = '__gcli_border';
const HIGHLIGHT_STYLE = '1px dashed black';
...
Highlighter.prototype._highlightNode = function(node) {
  ...
  const styles = this._document.defaultView.getComputedStyle(node);
...
Highlighter.prototype._unhighlightNode = function(node) {
  const previous = node.getAttribute(ATTR_NAME);
...
exports.toDom = function(document, html) {
  const div = util.createElement(document, 'div');
