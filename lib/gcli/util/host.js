...
var subprocess = require('resource:///modules/devtools/subprocess.jsm', {}).subprocess;
...
exports.spawn = function(context, spawnSpec) {
  return new Promise(function(resolve, reject) {
    console.debug('About to exec ' + spawnSpec.cmd +
                  ' with (' + spawnSpec.args.join(', ') + ')');
    ...
    subprocess.call({
      ...
      arguments: spawnSpec.args,
      mergeStderr: true,
      workdir: spawnSpec.cwd,
      charset: 'UTF-8',
      done: function(result) {
        console.debug('Result ', result);

        var output = { data: result.stdout, code: result.exitCode };
