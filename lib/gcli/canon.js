...
function Parameter(types, paramSpec, command, groupName) {
  ...
  if (this._defaultValue != null) {
    try {
      ...
      // in which case we'll need to mock one up in some way, or just stop
      // doing this startup check
      var context = null;
      var val = this.type.stringify(this.paramSpec.defaultValue, context);
      promise.resolve(val).then(function(defaultText) {
        var parsed = this.type.parseString(defaultText, context);
        return parsed.then(function(defaultConversion) {
...
Object.defineProperty(Parameter.prototype, 'json', {
  get: function() {
    // i.e. deep clone
    var json = JSON.parse(JSON.stringify(this.paramSpec));

    // Overwrite the type spec to allow remote eval
    json.type = this.type.getSpec();
    ...
    if (this.groupName != null && this.groupName !== this.paramSpec.option) {
      json.option = this.groupName;
    }
...
Canon.prototype.getCommandSpecs = function() {
  ...
  Object.keys(this._commands).forEach(function(name) {
    ...
    if (!command.noRemote) {
      commandSpecs.push(command.json);
    }
  }.bind(this));
