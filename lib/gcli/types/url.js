...
exports.items = [
  {
    ...
    parse: function(arg, context) {
      ...
      try {
        const url = host.createUrl(arg.text);
        ...
        if (arg.text.indexOf('://') === -1) {
          [ 'http', 'https' ].forEach(scheme => {
            try {
              const http = host.createUrl(scheme + '://' + arg.text);
          ...
          if ('window' in context.environment) {
            try {
              const base = context.environment.window.location.href;
              const localized = host.createUrl(arg.text, base);
