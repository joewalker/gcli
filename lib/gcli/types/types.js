...
Argument.prototype.beget = function(options) {
  ...
  if (options.text != null) {
    ...
    if (!options.dontQuote) {
      const needsQuote = text.indexOf(' ') >= 0 || text.length === 0;
      const hasQuote = /['"]$/.test(prefix);
  ...
  const ArgumentType = options.type || Argument;
...
Object.defineProperty(Argument.prototype, '_summaryJson', {
  get: function() {
    const assignStatus = this.assignment == null ?
            'null' :
...
ScriptArgument.prototype.beget = function(options) {
  ...
  const begotten = Argument.prototype.beget.call(this, options);
...
function MergedArgument(args, start, end) {
  ...
  const arg = Argument.merge(this.args);
  this.text = arg.text;
...
Object.defineProperty(MergedArgument.prototype, 'assignment', {
  ...
  set: function(assignment) {
    ...
    this.args.forEach(arg => arg.assignment = assignment);
  },
  enumerable: true
});
...
NamedArgument.prototype.beget = function(options) {
  ...
  const matches = /^([\s]*)([^\s]*)([\s]*['"]?)$/.exec(begotten.prefix);
...
const Status = {
   ...
   */
  combine: function() {
    let combined = Status.VALID;
    for (let i = 0; i < arguments.length; i++) {
      let status = arguments[i];
...
function ArrayConversion(conversions, arg) {
  ...
  this.value = conversions.map(conversion => conversion.value);

  this._status = Status.combine(conversions.map(conversion => conversion.getStatus()));
...
Object.defineProperty(ArrayConversion.prototype, 'assignment', {
  ...
  set: function(assignment) {
    ...
    this.conversions.forEach(conversion => conversion.assignment = assignment);
  },
  enumerable: true
});
...
ArrayConversion.prototype.toString = function() {
  return '[ ' + this.conversions.map(c => c.toString()).join(', ') + ' ]';
};
