...
JavascriptType.prototype.parse = function(arg) {
  ...
  // use. Initially look for matches directly on the object itself
  try {
    Object.keys(scope).forEach(function(property) {
      if (property.indexOf(matchProp) === 0) {
        matches[property] = { prop: property, ownProperty: true };
      }
    });
  }
  catch (ex) {
  ...
  // Use the prototype chain only if we can display more
  try {
    if (Object.keys(matches).length < JavascriptType.MAX_COMPLETION_MATCHES) {
      for (var prop in scope) {
        if (prop.indexOf(matchProp) === 0 && !(prop in matches)) {
          matches[prop] = { prop: prop, ownProperty: false };
        }
      }
    }
  }
  ...
  matches.sort(function(m1, m2) {
    if (m1.ownProperty !== m2.ownProperty) {
      return (m2.ownProperty ? 1 : 0) - (m1.ownProperty ? 1 : 0);
    }
