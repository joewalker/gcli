...
JavascriptType.prototype.parse = function(arg) {
  ...
  if (!isNaN(parseFloat(typed)) && isFinite(typed)) {
    return Q.resolve(new Conversion(typed, arg));
  }
  ...
  if (beginning.err) {
    return Q.resolve(new Conversion(typed, arg, Status.ERROR, beginning.err));
  }
  ...
  if (beginning.state !== ParseState.NORMAL) {
    return Q.resolve(new Conversion(typed, arg, Status.INCOMPLETE, ''));
  }
  ...
  if (properties.length > 1) {
    ...
    for (var i = 0; i < properties.length; i++) {
      ...
      if (scope == null) {
        return Q.resolve(new Conversion(typed, arg, Status.ERROR,
      ...
      try {
        ...
        return Q.resolve(new Conversion(typed, arg, Status.VALID, ''));
      }
    }
  }
  else {
  ...
  return Q.resolve(new Conversion(typed, arg, status, message,
