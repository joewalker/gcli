...
JavascriptType.prototype.parse = function(arg, context) {
  const typed = arg.text;
  let scope = (context.environment.window == null) ?
  ...
  // should be completed.
  const beginning = this._findCompletionBeginning(typed);
  ...
  const completionPart = typed.substring(beginning.startPos);
  ...
  if (scope == null) {
    const msg = l10n.lookupFormat('jstypeParseMissing', [ prop ]);
  ...
  const matchLen = matchProp.length;
  const prefix = matchLen === 0 ? typed : typed.slice(0, -matchLen);
  ...
  matches = Object.keys(matches).map(property => {
  ...
  let predictions = matches.map(match => {
    let description;
    let incomplete = true;
