...
exports.items = [
  ...
  },
  {
    ...
    parse: function(arg, context) {
      ...
      var predictFunc = function() {
        return this._findPredictions(arg).then(function(predictions) {
          ...
          if (command && command.name === arg.text &&
              typeof command.exec === 'function' && predictions.length === 1) {
            return [];
          }
      ...
      if (command) {
          //(command && (!this.allowNonExec && typeof command.exec !== 'function'))
        var s = typeof command.exec !== 'function' ?
            Status.INCOMPLETE :
            Status.VALID;
        var conversion = new Conversion(command, arg, s, '', predictFunc);
