...
CommandType.prototype._findPredictions = function(arg) {
  ...
  // Try infix matching if we get less than half max matched
  ...
  if (predictions.length === 0) {
    ...
    var names = lookup.map(function(itm) {
      return itm.name;
    });
    speller.train(names);
    var corrected = speller.correct(arg.text);
    if (corrected) {
      lookup.forEach(function(itm) {
        if (itm.name === corrected) {
          predictions.push(itm);
        }
      }, this);
    }
  }
