...
function SelectionType(typeSpec) {
  if (typeSpec) {
    Object.keys(typeSpec).forEach(key => this[key] = typeSpec[key]);
  }
...
function resolve(thing, context) {
  return Promise.resolve(thing).then(resolved => {
...
exports.convertPredictions = function(arg, predictions) {
  ...
  if (predictions[0].name === arg.text) {
    const value = predictions[0].value;
...
SelectionType.prototype.getBlank = function(context) {
  const predictFunc = context2 => {
    return Promise.resolve(this.getLookup(context2)).then(lookup => {
      return lookup.filter(option => !isHidden(option))
...
SelectionType.prototype.nudge = function(value, by, context) {
  return this.getLookup(context).then(lookup => {
    let index = this._findValue(lookup, value);
    if (index === -1) {
      if (by < 0) {
