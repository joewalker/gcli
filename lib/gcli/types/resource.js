...
CssResource._getStyle = function(domSheet, resources) {
  let resource = ResourceCache.get(domSheet);
  if (!resource) {
  ...
  try {
    Array.prototype.forEach.call(domSheet.cssRules, domRule => {
...
ScriptResource._getAllScripts = function(context) {
  ...
  const resources = Array.prototype.map.call(scriptNodes, scriptNode => {
    let resource = ResourceCache.get(scriptNode);
    if (!resource) {
...
function dedupe(resources, onDupe) {
  ...
  const names = {};
  resources.forEach(scriptResource => {
...
const ResourceCache = {
   ...
   */
  get: function(node) {
    for (let i = 0; i < ResourceCache._cached.length; i++) {
