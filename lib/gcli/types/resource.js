 ...
 */
function Resource(id, name, type, inline) {
  this.id = id;
  this.name = name;
  this.type = type;
  this.inline = inline;
...
CssResource.prototype.loadContents = function(callback) {
  callback(this._domSheet.ownerNode.innerHTML);
};

CssResource._cachedStyles = [];
...
ScriptResource.prototype.loadContents = function(callback) {
  if (this.inline) {
    callback(this._scriptNode.innerHTML);
  }
  else {
    ...
    xhr.open('GET', this._scriptNode.src, true);
    xhr.send();
  }
};

ScriptResource._cachedScripts = [];
