...
Completer.prototype._getCompleterTemplateData = function() {
  ...
  var promisedDirectTabText = Q.defer();
  var promisedArrowTabText = Q.defer();
  var promisedEmptyParameters = Q.defer();
  ...
  if (input.typed.trim().length !== 0) {
    prediction = current.getPredictionAt(this.choice);
  }

  Q.resolve(prediction).then(function(prediction) {
