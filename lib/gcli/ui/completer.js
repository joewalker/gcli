...
Completer.prototype._getCompleterTemplateData = function() {
  ...
  if (input.typed.trim().length !== 0) {
    ...
    if (prediction) {
      ...
      // (i.e. ' --name' not ' --name ')
      if (cArg.type === 'NamedArgument' && cArg.prefix.slice(-1) !== ' ') {
        tabText = ' ' + tabText;
      }
      ...
      if (cArg.type === 'NamedArgument' && cArg.text === '') {
        var prefix = (cArg.prefix.slice(-1) === ' ') ? '' : ' ';
        emptyParameters.push(prefix + '<' + current.param.type.name + '>');
      }
    }
  }
  ...
  // then we don't want any (extra) text for that parameter at all and the
  // algorithm to add spaces needs to take this into account.
  ...
  if (emptyParameters.length === 0) {
    ...
    if (command && command.hasNamedParameters && directTabText === '') {
