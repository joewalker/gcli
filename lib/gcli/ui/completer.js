 ...
 * - completeElement: Element to use as root
...
function Completer(components) {
  ...
  this.document = this.completeElement.ownerDocument;

  this.terminal = components.terminal;

  this.terminal.onInputChange.add(this.update, this);
  this.terminal.onAssignmentChange.add(this.update, this);
  this.terminal.onChoiceChange.add(this.update, this);

  this.autoResize = components.autoResize;
  if (this.autoResize) {
    this.terminal.onResize.add(this.resized, this);

    var dimensions = this.terminal.getDimensions();
    if (dimensions) {
...
Completer.prototype.destroy = function() {
  this.terminal.onInputChange.remove(this.update, this);
  this.terminal.onAssignmentChange.remove(this.update, this);
  this.terminal.onChoiceChange.remove(this.update, this);

  if (this.autoResize) {
    this.terminal.onResize.remove(this.resized, this);
  }
...
/**
 * Ensure that the completion element is the same size and the terminal element
 */
Completer.prototype.resized = function(ev) {
  this.completeElement.style.top = ev.top + 'px';
  this.completeElement.style.height = ev.height + 'px';
  this.completeElement.style.lineHeight = ev.height + 'px';
  this.completeElement.style.left = ev.left + 'px';
  this.completeElement.style.width = ev.width + 'px';
};
