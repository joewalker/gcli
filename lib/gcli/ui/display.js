...
function Display(options) {
  ...
  if (options.helper) {
    ...
    this.helper = options.useTooltip ?
            new Tooltip(options) :
            new ArgFetcher(options);
  }

  // A container to show either an Tooltip or a Menu depending on the state
  // of the requisition.
  if (displayCss != null) {
    this.style = util.dom.importCss(displayCss, this.document);
  }
  ...
  util.dom.setInnerHtml(this.element, displayHtml);
