...
exports.createView = function(options) {
  ...
  // it, then it's their job. See insertElement.
  var completeElement = insertElement(inputElement, options.completeElement,
                                      'gcli-row-complete');
  ...
  var prompt = new Prompt(options, {
    inputter: inputter,
    element: insertElement(inputElement, options.promptElement, 'gcli-prompt')
  });
 ...
 */
function insertElement(sibling, element, id) {
