...
const FocusManager = require('./focus').FocusManager;
 ...
 * @param options Object that links to other UI components. GCLI provided:
 * - requisition
 * - document
 */
Terminal.create = function(system, options) {
  ...
  return resourcesPromise.then(resources => {
    const terminal = Object.create(Terminal.prototype);
    return terminal._init(system, options, resources[0], resources[1]);
  });
};
 ...
 * @private
 */
Terminal.prototype._init = function(system, options, terminalCss, terminalHtml) {
  this.system = system;
  this.document = options.document || document;
  this.options = options;

  this.focusManager = new FocusManager(this.document, system.settings);
  ...
  const template = host.toDom(this.document, terminalHtml);
  ...
  const tooltipParent = template.querySelector('.gcli-tooltip');
  ...
  const completerParent = template.querySelector('.gcli-in-complete');
  ...
  this.displayElement.addEventListener('click', this.focus, false);
  ...
  return this.switchLanguage(null).then(() => this);
};
...
Terminal.prototype.destroy = function() {
  ...
  this.displayElement.removeEventListener('click', this.focus, false);
...
Terminal.prototype.switchLanguage = function(name) {
  ...
  return languages.createLanguage(name, this)
                  .then(language => this._updateLanguage(language));
};
...
Terminal.prototype.pushLanguage = function(name) {
  const languages = this.system.languages;
  return languages.createLanguage(name, this).then(language => {
...
Terminal.prototype.onOutput = function(ev) {
  ...
  const rowoutEle = this.document.createElement('pre');
  ...
  output += ' ' + ev.arguments.map(arg => arg).join(',');
...
Terminal.prototype.handleKeyUp = function(ev) {
  ...
  if (ev.keyCode === KeyEvent.DOM_VK_UP) {
    ...
    return this.language.handleUpArrow().then(handled => {
      if (!handled) {
        return this.nudgeChoice(1);
      }
    });
  ...
  if (ev.keyCode === KeyEvent.DOM_VK_DOWN) {
    ...
    return this.language.handleDownArrow().then(handled => {
      if (!handled) {
        return this.nudgeChoice(-1);
      }
    });
  ...
  if (ev.keyCode === KeyEvent.DOM_VK_RETURN) {
    if (this.isMenuShowing && this.field.menu.isSelected) {
      this.field.menu.clickSelected();
      return RESOLVED;
    }

    const input = this.inputElement.value;
    return this.language.handleReturn(input).then(handled => {
      if (!handled) {
        this._scrollingThroughHistory = false;
  ...
  if (ev.keyCode === KeyEvent.DOM_VK_TAB && !ev.shiftKey) {
    ...
    const hasContents = (this.inputElement.value.length > 0);
 ...
 */
Terminal.prototype.nudgeChoice = function(by) {
  if (this.field && this.field.menu) {
    this.field.menu.nudgeChoice(by);
  }
  return this.updateCompletion();
};
...
Terminal.prototype.updateCompletion = function() {
  return this.language.getCompleterTemplateData().then(data => {
    const template = this.completerTemplate.cloneNode(true);
...
Terminal.prototype.fieldChanged = function(ev) {
  ...
  this.document.defaultView.setTimeout(() => this.focus(), 10);
};
...
Terminal.prototype.scrollToBottom = function() {
  ...
  const scrollHeight = Math.max(this.displayElement.scrollHeight,
