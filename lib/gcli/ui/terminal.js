...
var CommandLanguage = require('../languages/command').CommandLanguage;
...
Terminal.prototype._init = function(components, terminalCss, terminalHtml) {
  ...
  // var outputViewTemplate = template.querySelector('.gcli-display > div');
  // outputViewTemplate.parentElement.removeChild(outputViewTemplate);
  var outputViewParent = template.querySelector('.gcli-display');
  var outputViewTemplate = outputViewParent.children[0];
  outputViewParent.removeChild(outputViewTemplate);
  ...
  var Language = languages.getLanguages()[0];
  this.language = new Language({
    terminal: this,
    focusManager: this.focusManager,
    requisition: components.requisition,
    outputViewTemplate: outputViewTemplate,
...
Terminal.prototype.handleKeyUp = function(ev) {
  ...
  if (ev.keyCode === KeyEvent.DOM_VK_RETURN) {
    return this.language.handleReturn();
  }
...
Terminal.prototype.clear = function() {
  while (this.displayElement.hasChildNodes()) {
    if (this.displayElement.firstChild === this.inputElement.parentElement) {
      break;
    }
