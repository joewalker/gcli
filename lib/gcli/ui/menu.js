...
Menu.prototype.onItemClickInternal = function(ev) {
  ...
  if (!name) {
    const named = ev.currentTarget.querySelector('[data-name]');
...
Object.defineProperty(Menu.prototype, 'selected', {
  get: function() {
    const item = this.element.querySelector('.gcli-menu-name.gcli-menu-highlight');
    if (!item) {
      return null;
    }
...
Menu.prototype.show = function(items, match) {
  ...
  const options = this.template.cloneNode(true);
  ...
  Object.defineProperties(proxy, {
    highlight: {
      get: function() {
        ...
        const startMatch = value.indexOf(match);
        if (startMatch === -1) {
          return value;
        }

        const before = value.substr(0, startMatch);
        const after = value.substr(startMatch + match.length);
        const parent = util.createElement(document, 'span');
        parent.appendChild(document.createTextNode(before));
        const highlight = util.createElement(document, 'span');
...
Menu.prototype._updateHighlight = function() {
  const names = this.element.querySelectorAll('.gcli-menu-name');
  const descs = this.element.querySelectorAll('.gcli-menu-desc');
  ...
  let index = this._choice % names.length;
  if (index < 0) {
