...
function History(options) {
    ...
    //
    // this._buffer[0] should always be equal to what the user has entered
    // before they started scrolling through history, but hasn't hit return on
    // yet. This way they can always come back if they decide that nothing in
    // the history suits their needs.
    this._buffer = [""];
...
History.prototype.add = function(command) {
    ...
    this._current = 0;
    // The temporary, non-submitted command doesn't get saved to the history, it
    // is now blank.
    this._buffer[0] = "";
};
...
History.prototype.update = function(partialCommand) {
    if (this._current === 0) {
        this._buffer[0] = partialCommand;
    }
}
