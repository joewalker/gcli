 ...
 * - outputElement: Root element to populate
...
function OutputTerminal(options, components) {
  this.outputElement = components.outputElement;
  this.requisition = components.requisition;

  this.requisition.commandOutputManager.onOutput.add(this.outputted, this);

  var document = components.outputElement.ownerDocument;
  if (outputViewCss != null) {
    this.outputViewStyle = util.importCss(outputViewCss, document, 'gcli-output-view');
  }

  this.outputViewTemplate = util.toDom(document, outputViewHtml);
}
...
OutputTerminal.prototype.destroy = function() {
  if (this.outputViewStyle) {
    this.outputViewStyle.parentNode.removeChild(this.outputViewStyle);
    this.outputViewStyle = undefined;
  }
...
function OutputView(output, outputTerminal) {
  ...
  var template = this.outputTerminal.outputViewTemplate.cloneNode(true);
  domtemplate.template(template, this, {
    allowEval: true,
    stack: 'output_terminal.html'
  });

  this.outputTerminal.outputElement.appendChild(this.elems.rowin);
  this.outputTerminal.outputElement.appendChild(this.elems.rowout);
...
OutputView.prototype.destroy = function() {
  ...
  this.outputTerminal.outputElement.removeChild(this.elems.rowin);
  this.outputTerminal.outputElement.removeChild(this.elems.rowout);

  this.output = undefined;
  this.outputTerminal = undefined;
  this.url = undefined;
...
OutputView.prototype.scrollToBottom = function() {
  ...
  var scrollHeight = Math.max(this.outputTerminal.outputElement.scrollHeight,
                              this.outputTerminal.outputElement.clientHeight);
  this.outputTerminal.outputElement.scrollTop =
                scrollHeight - this.outputTerminal.outputElement.clientHeight;
};
