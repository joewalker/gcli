...
function OutputView(output, outputTerminal) {
  ...
  this.output.onChange.add(this.changed, this);
}

OutputView.prototype.destroy = function() {
  this.output.onChange.remove(this.changed, this);
...
OutputView.prototype.changed = function(ev) {
  ...
  var node = converters.convert(this.output.data, this.output.type, 'dom',
                                this.outputTerminal.requisition.context);
