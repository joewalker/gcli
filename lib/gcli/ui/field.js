...
function SelectionField(doc, type, named, name, requ) {
  ...
  this.type = type;
  this.opts = {};
  this.defaultText = 'Select a ' + name + ' ...';
  ...
  Object.keys(lookup).forEach(function(key) {
    this._addOption(lookup[key], key);
  }, this);
...
SelectionField.claim = function(type) {
  // Turn SelectionField off while we mess with FilterField
 ...
 * </ul>
 */
function FilterField(doc, type, named, name, requ) {
  this.doc = doc;
  this.type = type;

  this.element = dom.createElement('div', null, this.doc);

  this.input = dom.createElement('input', null, this.doc);
  this.input.type = 'text';
  this.element.appendChild(this.input);
  this.onInputChange = this.onInputChange.bind(this);
  this.input.addEventListener('keyup', this.onInputChange, false);

  this.menu = new Menu(this.doc);
  this.element.appendChild(this.menu.element);
  this.menu.element.style.maxHeight = "300px";
  this.menu.element.style.overflowY = "auto";

  this.fieldChanged = createEvent('FilterField.fieldChanged');
}

FilterField.prototype = Object.create(Field.prototype);

FilterField.claim = function(type) {
  return type instanceof JavascriptType ? Field.MATCH : Field.NO_MATCH;
};

FilterField.prototype.destroy = function() {
  this.input.removeEventListener('change', this.onChange, false);
};

FilterField.prototype.setConversion = function(conversion) {
...
FilterField.prototype.getConversion = function() {
  var value = this.input.value === FilterField.DEFAULT_VALUE ?
      null :
      this.items[this.input.value].value;
  var arg = new Argument(this.type.stringify(value), ' ');
  return new Conversion(value, arg);
};

FilterField.DEFAULT_VALUE = '__FilterField.DEFAULT_VALUE';
