...
function SelectionField(doc, type, named, name, requ) {
  ...
  this.type = type;
  this.opts = {};
  this.defaultText = l10n.lookup('field_selection_select', { name: name });
...
function JavascriptField(doc, type, named, name, requ) {
  ...
  this.element = doc.createElement('div', null, this.doc);
...
JavascriptField.prototype.setConversion = function(conversion) {
  ...
  predictions.forEach(function(pair) {
    ...
    if (!value.hidden) {
      items.push({
        name: pair.name.substring(prefixLen),
        complete: pair.name,
...
function ArrayField(doc, type, named, name, requ) {
  ...
  this.addButton.innerHTML = l10n.lookup('field_array_add');
...
ArrayField.prototype._onAdd = function(ev, subConversion) {
  ...
  delButton.innerHTML = l10n.lookup('field_array_del');
