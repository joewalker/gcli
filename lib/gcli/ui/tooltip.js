...
function Tooltip(options) {
  ...
  dom.setInnerHtml(templates, tooltipHtml);
  this.template = templates.querySelector('.gcli-tt-template');
...
Tooltip.prototype.onAssignmentChange = function(ev) {
  ...
  if (this.assignment instanceof CommandAssignment) {
    ...
    // We can continue to document the assignment, but that's
  }
  ...
  this.field.fieldChanged.add(function(evt) {
    assignment.setConversion(evt.conversion);
  }, this);
  this.assignment.assignmentChange.add(function(evt) {
    this.field.setConversion(evt.conversion);
  }.bind(this));
