...
RequestView.startup = function(options) {
    ...
    RequestView.inputter = options.inputter;
    RequestView.requ = options.requ;

    RequestView.requestsView = new RequestsView(RequestView.doc);
...
RequestView.prototype = {
    ...
    copyToInput: function() {
        if (RequestView.inputter) {
            RequestView.inputter.setInput(this.request.typed);
        }
    },
    ...
    execute: function(ev) {
        if (RequestView.requ) {
            RequestView.requ.exec({ typed: this.request.typed });
        }
    },
