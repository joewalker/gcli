...
Inputter.prototype._setInputInternal = function(str) {
  if (this.element.value === str) {
    var input = this.getInputState();
    input = this._processCaretChange(input, false);
    dom.setSelectionStart(this.element, input.cursor.start);
    dom.setSelectionEnd(this.element, input.cursor.end);
    return;
  }
  ...
  this.doc.defaultView.setTimeout(function() {
    ...
    input.typed = str;
    input = this._processCaretChange(input, true);

    this.element.value = input.typed;
    dom.setSelectionStart(this.element, input.cursor.start);
    dom.setSelectionEnd(this.element, input.cursor.end);
  }.bind(this), 0);
};
...
Inputter.prototype._processCaretChange = function(input, forceUpdate) {
  ...
  switch (this._caretChange) {
    ...
    case Caret.TO_ARG_END:
      ...
      while (input.typed[start - 1] !== ' ' && start < input.typed.length);
...
Inputter.prototype.getInputState = function() {
  ...
  if (input.typed == null || (input.typed.length < input.cursor.start)) {
    var input = { typed: '', cursor: { start: 0, end: 0 } };
    console.log('hardcode fudge: this.requ.update=', input);
  }
