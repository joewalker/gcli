...
Inputter.prototype.onInputChange = function() {
    var requValue = this.requ.toString();
    // Only set the value if the value changed. Otherwise the cursor is
    // set to the end of the input field on every change.
    if (requValue != this.element.value) {
        this.element.value = requValue;
    }
    this.completer.update();
};
...
Completer.prototype.update = function() {
    ...
    var predictions = current ? current.getPredictions() : [];
