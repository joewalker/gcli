...
Inputter.prototype.onKeyUp = function(ev) {
    ...
    if (ev.keyCode === 13 /*RETURN*/) {
        ...
        // TODO: getCurrentAssignment() never returns null, so how can I figure
        // out if there is no current assignment? Best I have found is checking
        // against this.requ.commandAssignment, but I am not quite sure exactly
        // what that is and if you ever would want to actually increment or
        // decrement it (it throws an exception when I try to do either).
        if (currentAssignment !== this.requ.commandAssignment
            && !this._scrollingThroughHistory) {
            currentAssignment.increment();
        } else {
            this._scrollingThroughHistory = true;
            this.element.value = this.history.backward();
            this.update();
            // TODO: What should the selection do? Select all? Cursor at the end?
        }
    }
...
Inputter.prototype.update = function() {
    this.updateCli();
    this.completer.update();
    this.history.update(this.element.value);
};
