...
function Inputter(options) {
  ...
  if (inputterCss != null) {
    this.style = util.dom.importCss(inputterCss, this.document);
  }
  ...
  this.assignmentChange = util.createEvent('Inputter.assignmentChange');
...
Inputter.prototype._checkAssignment = function() {
  ...
  if (this.assignment !== newAssignment) {
    ...
    this.assignmentChange({ assignment: this.assignment });
  }
};
