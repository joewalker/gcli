...
function Inputter(options) {
  ...
  this.requisition.onInputChange.add(this.onInputChange, this);
...
Inputter.prototype.destroy = function() {
  this.requisition.onInputChange.remove(this.onInputChange, this);
  if (this.focusManager) {
...
/**
 * Handler for the Requisition.onInputChange event
 */
 ...
 * value, and makes no changes if they are the same (except for caret updating
 * - see below).
 * If changes are to be made, we use a timeout to avoid XUL bug 676520.
 * This function assumes that the data model is up to date with the new value.
 * It attempts to leave the caret in the same position in the input string
 * unless this._caretChange === Caret.TO_ARG_END. This is required for
 * completion events.
 */
Inputter.prototype._setInputInternal = function(str, update) {
  ...
  if (this.element.value && this.element.value === str) {
    this._processCaretChange(this.getInputState());
    return;
  }
...
Inputter.prototype.onKeyUp = function(ev) {
  ...
  if (ev.keyCode === KeyEvent.DOM_VK_TAB && !ev.shiftKey) {
    ...
    if (this.lastTabDownAt + 1000 > ev.timeStamp) {
      ...
      this.requisition.complete(inputState.cursor);
    }
    this.lastTabDownAt = 0;
    this._scrollingThroughHistory = false;
    return;
