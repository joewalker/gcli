...
function FocusManager(document, settings) {
  ...
  const eagerHelper = this.settings.get('eagerHelper');
...
FocusManager.prototype.destroy = function() {
  ...
  for (let i = 0; i < this._monitoredElements.length; i++) {
    const monitor = this._monitoredElements[i];
...
FocusManager.prototype.removeMonitoredElement = function(element, where) {
  ...
  this._monitoredElements = this._monitoredElements.filter(monitor => {
