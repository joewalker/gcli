...
/**
 * We only want to import popupCss once so this tracks whether or not we have
 ...
 */
function Popup(doc, options) {
    this.doc = doc;

    if (!popupCssImported) {
        dom.importCssString(popupCss, this.doc);
        popupCssImported = true;
    }
    ...
    this.children.forEach(function(child) {
        if (child.element) {
            this.output.appendChild(child.element);
        }
    }, this);
