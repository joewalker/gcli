...
exports.maybeShowIntro = function(commandOutputManager, conversionContext) {
  const hideIntro = conversionContext.system.settings.get('hideIntro');
  if (hideIntro.value) {
    return;
  }

  const output = new Output(conversionContext);
  output.type = 'view';
  commandOutputManager.onOutput({ output: output });

  const viewData = this.createView(null, conversionContext, true);
...
exports.createView = function(ignoreArgs, conversionContext, showHideButton) {
  return view.createView({
    ...
    data: {
      ...
      onGotIt: function(ev) {
        const settings = conversionContext.system.settings;
        const hideIntro = settings.get('hideIntro');
