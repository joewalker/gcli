...
function CommandOutputListView(element, options) {
  ...
  if (commandOutputViewCss != null) {
    this.style = util.importCss(commandOutputViewCss, this.document);
  }

  this.template = util.toDom(this.document, commandOutputViewHtml);
...
CommandOutputView.prototype.onChange = function(ev) {
  ...
  util.clearElement(this.elems.output);
  ...
  if (output != null) {
    ...
    if (isElement) {
      ...
      node = util.createElement(this.listView.document, 'p');
      util.setInnerHtml(node, output.toString());
