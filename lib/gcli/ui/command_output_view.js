...
function CommandOutputListView(element, options) {
  ...
  if (commandOutputViewCss != null) {
    this.style = util.dom.importCss(commandOutputViewCss, this.document);
  }

  this.template = util.dom.toDom(this.document, commandOutputViewHtml);
...
function CommandOutputView(outputData, commandOutputListView) {
  ...
  this.element = this.listView.template.cloneNode(true);
  domtemplate.template(this.element, this, this.listView.templateOptions);
...
CommandOutputView.prototype.onChange = function(ev) {
  var duration = this.outputData.duration != null ?
          ...
          '';
  duration = this.listView.document.createTextNode(duration);
  ...
  util.dom.clearElement(this.elems.output);
  ...
  if (output != null) {
    ...
    if (isElement) {
      ...
      node = util.dom.createElement(this.listView.document, 'p');
      util.dom.setInnerHtml(node, output.toString());
