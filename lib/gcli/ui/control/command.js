 ...
 */
function CommandControl(terminal, focusManager, requisition) {
...
CommandControl.prototype.textChanged = function() {
  ...
  if (this.assignment.arg.text === this.terminal.lastText) {
    return;
  }

  this.terminal.lastText = this.assignment.arg.text;
...
CommandControl.prototype.caretMoved = function(start) {
  ...
  if (this.terminal.focusManager) {
    this.terminal.focusManager.setError(this.assignment.message);
  }
};
...
CommandControl.prototype.handleDownArrow = function() {
  // See comments in handleUpArrow
  if (this.terminal.isMenuShowing) {
    this.terminal.decrementChoice();
    return RESOLVED;
  }

  if (this.terminal.inputElement.value === '' || this.terminal._scrollingThroughHistory) {
    this.terminal._scrollingThroughHistory = true;
    return this.requisition.update(this.terminal.history.forward());
  }
 ...
 */
CommandControl.prototype._getCompleterTemplateData = function() {
  var input = this.terminal.getInputState();
...
CommandControl.prototype.fieldChanged = function(ev) {
  ...
  this.terminal.focusManager.setError(isError);
};
...
CommandControl.prototype.outputted = function(ev) {
  ...
  var view = new OutputView(ev.output, this.terminal);
  ev.output.view = view;

  this.terminal.addElement(view.elems.rowin);
  this.terminal.addElement(view.elems.rowout);
...
function OutputView(output, terminal) {
  ...
  var context = this.terminal.control.requisition.conversionContext;
