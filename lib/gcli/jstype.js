...
JavascriptType.prototype.parse = function(arg) {
    ...
    for (var i = 0; i < properties.length; i++) {
      ...
      if (scope == null) {
        return new Conversion(typed, arg, Status.ERROR,
                l10n.lookup('jstype_parse_scope'));
      }
  ...
  if (scope == null) {
    return new Conversion(typed, arg, Status.ERROR,
        l10n.lookupFormat('jstype_parse_missing', [ prop ]));
  }
  ...
  if (status !== Status.VALID) {
    message = l10n.lookupFormat('jstype_parse_missing', [ matchProp ]);
  }
    ...
    switch (state) {
      ...
      case ParseState.NORMAL:
        if (c === '"') {
          ...
          if (!last || OPEN_CLOSE_BODY[last.token] != c) {
            return { err: l10n.lookup('jstype_begin_syntax') };
          }
          if (c === '}') {
            start = i + 1;
          }
      ...
      case ParseState.DQUOTE:
        if (c === '\\') {
          ...
          return { err: l10n.lookup('jstype_begin_unterm') };
        }
        else if (c === '"') {
