...
const Connectors = require('./connectors/connectors').Connectors;
const Converters = require('./converters/converters').Converters;
const Fields = require('./fields/fields').Fields;
const Languages = require('./languages/languages').Languages;
const Settings = require('./settings').Settings;
const Types = require('./types/types').Types;
...
exports.createSystem = function(options) {
  ...
  const unloadModule = function(name) {
    const existingModule = loadedModules[name];
  ...
  const loadModule = function(name) {
    const existingModule = loadedModules[name];
    unloadModule(name);

    // And load the new items
    try {
      const loader = loadableModules[name];
      return Promise.resolve(loader(name)).then(newModule => {
        ...
        if (newModule.items == null || typeof newModule.items.forEach !== 'function') {
          ...
          throw new Error('Module \'' + name + '\' has no \'items\' array export');
        }
  ...
  const system = {
    ...
    addItemsByModule: function(names, opts) {
      const promises = [];
      ...
      names.forEach(name => {
        ...
        //   };
        // }
        loadableModules[name] = opts.loader;
    ...
    load: function() {
      ...
      const modules = Object.keys(loadedModules).map(name => loadedModules[name]);

      const promises = Object.keys(loadableModules).map(name => {
        delete modules[name];
    ...
    destroy: function() {
      ...
      Object.keys(loadedModules).forEach(name => unloadModule(name));
...
function syncItems(system, front, customProps) {
  return front.specs(customProps).then(specs => {
    removeItemsFromFront(system, front);

    const remoteItems = addLocalFunctions(specs, front);
...
function addLocalFunctions(specs, front) {
  ...
  specs.forEach(commandSpec => {
    ...
    commandSpec.params.forEach(param => {
    ...
    if (!commandSpec.isParent) {
      commandSpec.exec = function(args, context) {
        const typed = (context.prefix ? context.prefix + ' ' : '') + context.typed;
...
function removeItemsFromFront(system, front) {
  system.commands.getAll().forEach(command => {
