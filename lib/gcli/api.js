...
exports.getApi = function() {
  ...
  /**
   * addItemsByModule and removeItemsByModule need a store of modules that
   * have been added.
   * An optimization to this would be to ensure that we can store the items
   * (or the names) from each module.
   */
  var modules = {};
  ...
  var api = {
    ...
    addItemsByModule: function(name) {
      this.removeItemsByModule(name);

      var module = require(name);
      if (module == null) {
        throw new Error('No requireable module called \'' + name + '\'');
      }

      if (Array.isArray(module.items)) {
        throw new Error('Module \'' + name + '\' has no \'items\' export');
      }

      module[name] = module;
      module.items.forEach(addItem);
    },

    removeItemsByModule: function(name) {
      var oldModule = modules[name];
      if (oldModule != null) {
        oldModule.items.forEach(removeItem);
      }
      delete modules[name];
    },
  };
