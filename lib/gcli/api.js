...
var centralCommands = require('./commands/commands').centralCommands;
...
exports.createSystem = function() {
  ...
  settings.startup(components.type);

  var addItem = function(item) {
    // Some items are registered using the constructor so we need to check
    // the prototype for the the type of the item
    var itemType = item.item || item.prototype.item;
    components[itemType].add(item);
  };

  var removeItem = function(item) {
    var itemType = item.item || item.prototype.item;
    components[itemType].remove(item);
  };
  ...
  Object.defineProperty(api, 'types', {
    ...
    set: function(types) {
      ...
      settings.startup(components.types);
    },
    enumerable: true
  });
...
/**
 * api.createSystem() is clean, but generally we want to add the functions to
 * the 'exports' object. So this is a quick helper.
 */
exports.populateApi = function(obj) {
  var exportable = exports.createSystem();
