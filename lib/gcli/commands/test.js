...
const stati = require('../testharness/status').stati;
const helpers = require('../test/helpers');
const suite = require('../test/suite');
const cli = require('../cli');
const Requisition = require('../cli').Requisition;
const createRequisitionAutomator = require('../test/automators/requisition').createRequisitionAutomator;
...
exports.items = [
  {
    ...
    cacheable: true,
    lookup: function() {
      return Object.keys(examiner.suites).map(name => {
  ...
  },
  {
    ...
    exec: function(args, context) {
      ...
      return args.suite.run(options).then(() => {
        requisition.system.commands.get('mocks').off(requisition);
        const output = context.typedData('examiner-output', examiner.toRemote());
