  ...
  Object.defineProperty(command, 'paramGroups', {
    get: function() {
      ...
      this.params.forEach(param => {
        const groupName = param.option || l10n.lookup('canonDefaultGroupName');
...
function getHelpManData(commandData, context) {
  ...
  commandData.command.params = commandData.command.params.filter(param => {
    return !param.hidden;
  });
  ...
  return {
    ...
    getTypeDescription: function(param) {
      ...
      if (param.defaultValue === undefined) {
        ...
        /*
        const val = param.type.stringify(param.defaultValue);
        input = Promise.resolve(val).then(defaultValue => {
          return l10n.lookupFormat('helpManDefault', [ defaultValue ]);
        });
        */
      }

      return Promise.resolve(input).then(defaultDescr => '(' + (param.type.name || param.type) + ', ' + defaultDescr + ')');
    },
    getSynopsis: function(param) {
      const name = param.name + (param.short ? '|-' + param.short : '');
...
function getMatchingCommands(context, prefix) {
  ...
  let reply = commands.getAll().filter(command => {
    if (command.hidden) {
      return false;
    }
  ...
  reply.sort((c1, c2) => c1.name.localeCompare(c2.name));
  reply = reply.map(command => command.toJson());
...
function getSubCommands(context, command) {
  const commands = cli.getMapping(context).requisition.system.commands;
  let subcommands = commands.getAll().filter(subcommand => {
    return subcommand.name.indexOf(command.name) === 0 &&
           subcommand.name !== command.name &&
           !subcommand.hidden;
  });

  subcommands.sort((c1, c2) => c1.name.localeCompare(c2.name));
  subcommands = subcommands.map(subcommand => subcommand.toJson());
...
exports.items = [
  {
    ...
    exec: function(args, context) {
      const commands = cli.getMapping(context).requisition.system.commands;
      const command = commands.get(args.search);
      if (command) {
