  ...
  return {
    ...
    getTypeDescription: function(param) {
      ...
      return promise.resolve(input).then(function(defaultDescr) {
        return '(' + param.type.name + ', ' + defaultDescr + ')';
      }.bind(this));
    },
...
function getMatchingCommands(context, prefix) {
  ...
  // We should be converting commands to JSON here, something like:
  //   commands = commands.map(command => { return command.toJson(); });
  // Except the JSON version gets some parameter properties wrong
    ...
    exec: function(args, context) {
      ...
      if (command) {
        return context.typedData('commandData', {
          command: command, // See JSON comment above
