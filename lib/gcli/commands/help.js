...
var helpCommandSpec = {
  ...
  exec: function(args, context) {
    ...
    var parent = util.createElement(context.document, 'div');
    if (!args.search) {
...
help.onFirstUseStartup = function(document) {
  if (!helpIntroTemplate) {
    helpIntroTemplate = util.toDom(document, helpIntroHtml);
  }
  if (!helpListTemplate) {
    helpListTemplate = util.toDom(document, helpListHtml);
  }
  if (!helpManTemplate) {
    helpManTemplate = util.toDom(document, helpManHtml);
  }
...
function updateCommand(element, context) {
  ...
  context.requisition.update(typed);
}
