...
PrefList.prototype.updateTable = function() {
  util.dom.clearElement(this.table);
...
PrefList.onFirstUseStartup = function(document) {
  if (!PrefList.outerTemplate) {
    PrefList.outerTemplate = util.dom.toDom(document, PrefList.outerHtml);
  }

  if (!PrefList.innerTemplate) {
    PrefList.innerTemplate = util.dom.toDom(document, PrefList.innerHtml);
  }

  if (!PrefList.style && PrefList.css != null) {
    PrefList.style = util.dom.importCss(PrefList.css, document);
  }
};
