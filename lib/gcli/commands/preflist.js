 ...
 */
const prefsViewConverter = {
  item: 'converter',
  from: 'prefsData',
  to: 'view',
  exec: function(prefsData, conversionContext) {
    const prefList = new PrefList(prefsData, conversionContext);
    return {
      html:
 ...
 * Format a list of settings for display
 */
const prefsStringConverter = {
  item: 'converter',
  from: 'prefsData',
  to: 'string',
  exec: function(prefsData, conversionContext) {
    let reply = '';
    prefsData.settings.forEach(setting => {
...
PrefList.prototype.onLoad = function(element) {
  const table = element.querySelector('.gcli-pref-list-table');
...
PrefList.prototype.updateTable = function(table) {
  const view = this.conversionContext.createView({
    html:
      '<table>\n' +
      '  <colgroup>\n' +
      '    <col class="gcli-pref-list-name"/>\n' +
...
PrefList.prototype.onFilterChange = function(ev) {
  if (ev.target.value !== this.search) {
    ...
    const root = ev.target.parentNode.parentNode;
    const table = root.querySelector('.gcli-pref-list-table');
    this.updateTable(table);
  }
};

PrefList.prototype.onSetClick = function(ev) {
  const typed = ev.currentTarget.getAttribute('data-command');
