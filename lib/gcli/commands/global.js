...
exports.items = [
  {
    ...
    lookup: function(context) {
      const knownWindows = context.environment.window == null ?
                         [ ] : [ context.environment.window ];

      this.last = findWindows(knownWindows).map(window => {
...
function findWindows(knownWindows) {
  knownWindows.forEach(window => addChildWindows(window, knownWindows));
  return knownWindows;
}
