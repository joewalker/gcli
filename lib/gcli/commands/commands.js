...
function lookup(data, onUndefined) {
  ...
  if (typeof data === 'object') {
    ...
    const locales = l10n.getPreferredLocales();
    let translated;
...
function Command(types, commandSpec) {
  Object.keys(commandSpec).forEach(key => this[key] = commandSpec[key]);
  ...
  const paramSpecs = this.params;
  this.params = [];
  this.paramGroups = {};
  this._shortParams = {};

  const addParam = param => {
    const groupName = param.groupName || l10n.lookup('canonDefaultGroupName');
  ...
  paramSpecs.forEach(spec => {
    if (!spec.group) {
      const param = new Parameter(types, spec, this, null);
      addParam(param);
      ...
      spec.params.forEach(ispec => {
        const param = new Parameter(types, ispec, this, spec.group);
        addParam(param);
...
Commands.prototype.add = function(commandSpec) {
  ...
  if (this._commands[commandSpec.name] != null) {
    ...
    this._commandNames = this._commandNames.filter(test => {
      return test !== commandSpec.name;
    });
  }

  const command = new Command(this.types, commandSpec);
...
Commands.prototype.remove = function(commandOrName) {
  const name = typeof commandOrName === 'string' ?
          commandOrName :
          commandOrName.name;
  ...
  this._commandNames = this._commandNames.filter(test => test !== name);
...
Commands.prototype.getAll = function() {
  return Object.keys(this._commands).map(name => this._commands[name]);
};
...
Commands.prototype.addProxyCommands = function(commandSpecs, remoter, prefix, to) {
  ...
  commandSpecs.forEach(commandSpec => {
    const originalName = commandSpec.name;
